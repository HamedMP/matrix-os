...
- TypeScript 5.5+ strict, ES modules, Node.js 24+; Swift 6 (macOS app, observe-only in this feature) + Hono (platform/gateway), Next.js 16 + React 19 (shell, www), Kysely/Postgres (platform DB), Zod 4 via `zod/v4`, Clerk (identity), Stripe (billing), citty (CLI in `packages/sync-client`), Expo 55 / React Native (`apps/mobile`), Hetzner Cloud API (provisioning) (092-onboarding-unification)
- Platform PostgreSQL via Kysely — existing `user_machines`, `billing_entitlements`, `billing_customers`; new `billing_checkout_attempts`, `onboarding_first_run`, `onboarding_journey_events`. Inline migrations in `packages/platform/src/db.ts` `migrate()` (existing convention). VPS-local file `~/system/onboarding-complete.json` becomes a derived artifact, no longer the source of truth. (092-onboarding-unification)
