...
private struct Sidebar: View {
    ...
    private var newTaskButton: some View {
        Button {
            if model.section == .terminal { model.createSession() } else { model.createTask(status: .todo) }
        } label: {
            HStack(spacing: Spacing.x3) {
                Image(systemName: "plus")
                    .font(.system(size: 14, weight: .semibold))
                Text(model.section == .terminal ? "New session" : "New task / New agent")
                    .font(.plexSans(14, weight: .semibold))
                Spacer()
                Text(model.section == .terminal ? "⌘T" : "⌘N")
