...
function retryCurrentHandoff(): void {
  ...
  return (
    ...
    >
      <AuthAssetImage
        ...
        width={56}
        height={74}
        className="mb-7 h-[74px] w-14 object-contain"
      />
      {timedOut ? (
        <>
        ...
        <>
          <Loader2Icon
            className="mb-4 size-5 animate-spin"
            style={{ color: c.ember }}
            aria-hidden="true"
          />
          <h2 className="text-xl font-semibold tracking-tight" style={{ color: c.deep }}>
...
export function SignupBillingHandoff({
  startedAt,
  onRetry = retryCurrentHandoff,
}: {
  startedAt: number;
  onRetry?: () => void;
}) {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      // react-doctor-disable-next-line react-hooks-js/set-state-in-effect -- one-shot elapsed-time boundary drives the inline retry state
      setTimedOut(true);
    }, remainingMs);
  ...
  return (
    <div data-matrix-signup-billing-handoff="true">
      <AuthLayout
        ...
        formContent={<BillingHandoffCard timedOut={timedOut} onRetry={onRetry} />}
      />
    </div>
  );
}
