...
export function ChatSharing({ chatId }: { chatId: string }) {
  ...
  const collaborationApi = useMemo(createShellCollaborationApi, []);
  useEffect(() => {
    let active = true;
  ...
  return <ChatSharingButton api={api} collaborationApi={collaborationApi ?? undefined} runtimeId={runtime.runtimeId}
