...
- **Provider settings use precise vocabulary**: a harness is the executable agent runtime (Hermes, OpenClaw, Pi, OpenCode, Codex, Claude); a model provider serves inference (Matrix AI, Anthropic, OpenAI, OpenRouter, Baseten); an account is an owner-scoped login/profile; an access source is the exact credential and funding path used for a run. Internal V3 `ProviderDriver`/`ProviderInstance` names project harnesses, but new user-facing copy must not call a harness a model provider. See `docs/dev/provider-settings-parity.md`.
...
- **Provider settings have three-shell parity**: Canvas, Web Desktop, and Electron expose the same states and actions from shared schemas, derivations, clients, and feature components. Electron Desktop is the visual/interaction ground truth while converging, but validate Canvas first, then Web Desktop, then Electron. A shell adapter may supply chrome/navigation/Terminal launch only; silent feature omission or duplicated business logic is not allowed.
