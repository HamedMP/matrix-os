...
- **Managed backend rollout**: `backend_management_policy` defaults disabled. After a reviewed bridge/canary activation, the platform reconciler follows approved `stable` bundles with durable leases, observed-version/service-health checks, bounded cohorts and expiring support holds. Managed bridge hosts disable passive channel installs; operators still have explicit immutable deploys. Desktop/mobile minimums are separate client policies. Follow `docs/dev/managed-backend.md`; never silently enable the legacy fleet or raise client minimums before replacement installers/store builds are available.
