...
- **Cloudflare usage-mode billing is response-settled**: the managed GLM route uses the exact bounded usage returned by Cloudflare Workers AI. Admit only against an atomic affordable hold, permit one funded request in flight per owner, cap the user's debit at the hold, and record any final overrun as Matrix-absorbed rather than debt or a later-credit clawback. Missing final usage stays unresolved and fail-closed until exact reconciliation.
- **Starter credit is owner/campaign scoped**: a configured campaign defaults to $5 and may be explicitly set to $10. Claim it automatically only after live policy eligibility, once per owner/campaign across runtimes. Do not bulk-grant historical owners or infer credit from Cloudflare balance.
- **Managed generic default is GLM Flash**: new Pi/OpenCode Matrix AI routes default to `@cf/zai-org/glm-5.3-flash` through Cloudflare Workers AI. Preserve existing explicit Chat, harness, and own-account selections; do not route the Anthropic-only Claude harness through the OpenAI-compatible GLM path.
...
- **A request that produced nothing after 15 minutes was lost, not slow -- request once more.** Observed turnaround is about three minutes, so 15 is well past the normal range. Without this, a dropped request deadlocks the PR: the merge gate needs a 5/5 naming that exact commit, and the no-duplicate rule would forbid ever asking again. If a second request also produces nothing, that is an outage rather than a retry problem -- say so in the PR and escalate instead of continuing to ask.
