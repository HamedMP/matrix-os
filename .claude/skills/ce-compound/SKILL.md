...
If the line above resolved to a plain branch name (like `feat/my-branch`), include it in the optional session-history notes if a local session-history capability is available. If it still contains a backtick command string or is empty, omit it.
...
If the user says yes, run Phase 1 step 4 only when a local session-history capability is available in the current harness. This vendored Matrix OS skill set does not include `ce-sessions`; when it is unavailable, tell the user "Session-history search is not installed in this vendored skill set; continuing without it" and continue. If no, skip it. Do not ask this in lightweight mode or headless mode.
...
- **Then** run the optional session-history step (see step 4 below) — only if the user opted in and the current harness exposes a local session-history capability. This vendored Matrix OS skill set does not include `ce-sessions`, so the fallback is to continue without session history rather than invoking a missing skill. If a compatible session-history skill is installed, invoke it after launching the background subagents so the wall-clock benefit is preserved (`max(session-history search, slowest background subagent)`, not their sum). Issuing the search before the parallel block would serialize it in front of the research subagents and regress wall-clock time.
...
#### 4. **Optional Session History** (synchronous capability call, after launching the parallel block — only if the user opted in)
   ...
   - This vendored Matrix OS skill set intentionally does not include `ce-sessions`. Before invoking any session-history skill, verify that the current harness exposes it. If it is unavailable, state "Session-history search is not installed in this vendored skill set; continuing without it" and proceed without session-history findings. Do not attempt to invoke `ce-sessions` when it is not present.
   - If a compatible session-history capability is installed, invoke it via the platform's skill-invocation primitive (`Skill` in Claude Code, `Skill` in Codex, the equivalent on Gemini/Pi). Pass the dispatch payload below as the skill argument string. The session-history capability runs in main context — it owns discovery, branch/keyword filtering, scan-window selection, the deep-dive cap, per-session extraction to a `mktemp` scratch dir, and any synthesis-only subagent dispatch. The compound orchestrator only needs to pass the topic and time window and read back the findings text.

   **Dispatch payload — keep tight.** A long, keyword-rich payload licenses session-history search to keep widening. Use this shape:
   ...
   Do not append additional context blocks, exclusion lists, or topic-keyword bullets — verbose payloads give the session-history capability license to keep widening the search and rapidly compound wall time. If keyword search is needed, the session-history capability owns that decision internally based on the topic.
...
3. **Incorporate session history findings** (if available). When the optional session-history capability returned relevant prior-session context:
...
- This vendored Matrix OS skill set intentionally does not include `ce-compound-refresh`. Before invoking it, verify that the current harness exposes that skill. If it is unavailable, surface the narrow scope hint as a one-line recommendation and skip the invocation.
- If there is **one obvious stale candidate**, invoke `ce-compound-refresh` with a narrow scope hint after the new learning is written only when that skill is available; otherwise recommend the same scope hint as the next step
- If there are **multiple candidates in the same area**, ask the user whether to run a targeted refresh for that module, category, or pattern set only when `ce-compound-refresh` is available; otherwise surface the recommended scope hint
...
This vendored Matrix OS skill set intentionally does not include the specialized Phase 3 reviewer skills listed below. Before invoking any specialized reviewer, verify that the current harness exposes it. If none are available, state "Specialized ce-compound reviewers are not installed in this vendored skill set; skipping Phase 3" and continue. If only some are available, invoke only the available reviewers and skip the rest without failing.

<parallel_tasks>

These specialized reviewers are optional external capabilities, not bundled dependencies. Based on problem type, invoke only reviewers that are installed in the current harness:

- **performance_issue** → `ce-performance-oracle`
- **security_issue** → `ce-security-sentinel`
- **database_issue** → `ce-data-integrity-guardian`
- Any code-heavy issue → run `ce-code-simplicity-reviewer` for minimal, clear examples when that reviewer is available. Structural concerns in the diff are already covered when the same work goes through `/ce-code-review` (maintainability persona).
...
Subagent Results:
  ✓ Context Analyzer: Identified performance_issue in gateway_route, category: performance-issues/
...
issues occur in the Gateway Route or Platform Control Plane modules.
