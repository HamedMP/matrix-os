...
| `plan:<path>` | `plan:specs/082-paid-beta-readiness/plan.md` | Load this plan for requirements verification |
...
| `ce-data-migration-reviewer` | Matrix Kysely migrations (`packages/**/migrations/*.ts`, `packages/**/migrations/*.js`, `db/migrations/**`), schema dumps (`db/schema.rb`, `structure.sql`), backfills, data-transform scripts — **not** model/query-only changes without migration artifacts |
...
- `specs/*/plan.md` and `specs/*/spec.md` -- Matrix OS plan/spec files (decision artifacts; execution progress is derived from git, not stored in plan bodies)
...
2. **PR body.** If PR metadata was fetched in Stage 1, scan the body for paths matching `specs/*/plan.md`, `specs/*/spec.md`, or `specs/*.md`. If exactly one match is found and the file exists, use it as `plan_source: explicit`. If multiple spec paths appear, treat as ambiguous — demote to `plan_source: inferred` for the most recent match that exists on disk, or skip if none exist or none clearly relate to the PR title/intent. Always verify the selected file exists before using it — stale or copied plan links in PR descriptions are common.
3. **Auto-discover.** Extract 2-3 keywords from the branch name (e.g., `feat/onboarding-skill` -> `onboarding`, `skill`). Glob `specs/*/plan.md`, `specs/*/spec.md`, and `specs/*.md`, then filter paths containing those keywords. If exactly one match, use it. If multiple matches or the match looks ambiguous (e.g., generic keywords like `review`, `fix`, `update` that could hit many specs), **skip auto-discovery** — a wrong plan is worse than no plan. If zero matches, skip.
...
**`data-migration` spawn gate.** Select `ce-data-migration-reviewer` only when the diff includes at least one migration or schema artifact: Matrix Kysely TypeScript/JavaScript migrations such as `packages/**/migrations/*.ts`, `packages/**/migrations/*.js`, and `db/migrations/**`; legacy schema dumps such as `db/schema.rb` or `db/structure.sql`; Alembic/Flyway/Liquibase migration paths; or explicit backfill/data-transform scripts. **Do not spawn** for model-only changes, query-only refactors, serializers/controllers that reference columns without a migration or schema dump in the diff, or migration tests alone.

For `ce-deployment-verification-agent`, use the same migration-artifact gate when the change is risky (destructive DDL, backfills, NOT NULL without default, column renames/drops), but only after verifying that the current harness exposes that agent. This vendored Matrix OS skill set intentionally does not include it. If it is unavailable, skip the spawn and ask the `data-migration` persona to cover Go/No-Go deployment checks, verification queries, rollback caveats, and monitoring notes.
...
- data-migration -- adds Kysely migration packages/platform/migrations/20260528_add_release_channel.ts
...
**CE conditional agents** (`ce-deployment-verification-agent` only) are dispatched as standard Agent calls through the same bounded parallel scheduler only when the agent is available and the migration-artifact gate applies. Pass the same review context bundle plus the applicability reason (for example, which migration files triggered the agent). If the agent is unavailable, do not spawn it; record that it was skipped and route deployment verification expectations to `data-migration`. Agent output is unstructured and must be preserved for Stage 6 synthesis just like the CE always-on agents. Schema drift is handled by the `data-migration` persona as structured findings — not here.
...
5. **Protected artifacts are respected.** Discard any findings that recommend deleting or gitignoring files in `specs/`, `docs/brainstorms/`, or `docs/solutions/`.
