...
| `data-migration` | `ce-data-migration-reviewer` | Matrix Kysely migrations (`packages/**/migrations/*.ts`, `packages/**/migrations/*.js`, `db/migrations/**`), schema dumps (`db/schema.rb`, `structure.sql`), backfill scripts, data transformations — **not** model/query-only changes without migration artifacts |
...
This vendored Matrix OS skill set intentionally does not include `ce-deployment-verification-agent`. Before spawning it, verify that the current harness exposes that agent. If it is unavailable, skip the spawn and ask the `data-migration` persona to include Go/No-Go deployment checks, verification queries, rollback caveats, and monitoring notes in its structured findings.

Spawn `ce-deployment-verification-agent` only when it is available, the migration-artifact gate applies, and the change is risky (destructive DDL, backfills, NOT NULL without default, column renames/drops). Schema drift and migration safety live in the `data-migration` persona — not separate CE agents.
...
4. **For `data-migration`**, spawn only when the diff includes migration or schema artifacts (`packages/**/migrations/*.ts`, `packages/**/migrations/*.js`, `db/migrations/**`, `db/schema.rb`, `db/structure.sql`, Alembic/Flyway/Liquibase paths, or explicit backfill/data-transform scripts). Do **not** spawn for model-only or query-only changes without those files.
5. **For CE conditional agents**, spawn `ce-deployment-verification-agent` only when the agent is available, the migration-artifact gate applies, and the change is risky (see above). If unavailable, skip the agent and route the deployment checklist expectation to `data-migration`.
