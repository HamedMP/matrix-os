...
1. **Read Plan and Clarify** _(skip if arriving from Phase 0 with a bare prompt)_
   ...
   - When auto-detecting the latest plan (blank invocation), glob `specs/*/plan.md`, `specs/*/spec.md`, and `specs/*.md`, then pick the most recent matching file. Matrix OS feature planning lives under `specs/`.
...
6. **Figma Design Sync** (if applicable)
   ...
   - If `ce-figma-design-sync` or an equivalent visual comparison skill is available, use it iteratively to compare; otherwise compare manually against the design specs
