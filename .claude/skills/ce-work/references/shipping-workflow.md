...
3. **Commit and Create Pull Request**
   ...
   This vendored Matrix OS skill set intentionally does not include `ce-commit-push-pr`. Before loading it, verify that the current harness exposes that skill. If it is unavailable, perform the equivalent manual flow with the repo's native tools: inspect `git status`, stage only in-scope files, create conventional commits, push the current branch, and create or update the pull request with the same summary, tests, invariants, and residual-work context described below.

   When available, load the `ce-commit-push-pr` skill to handle committing, pushing, and PR creation. The skill handles convention detection, branch safety, logical commit splitting, adaptive PR descriptions, and attribution badges.
...
- [ ] Evidence decision handled by `ce-commit-push-pr` when available, or manually documented in the PR body when the change has observable behavior
