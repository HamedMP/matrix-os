...
**Dating rule:** derive today's date from the runtime environment when updating `last_updated` (for example, `date +%Y-%m-%d` when a shell is available). Do not hardcode a calendar year in generated strategy documents.
