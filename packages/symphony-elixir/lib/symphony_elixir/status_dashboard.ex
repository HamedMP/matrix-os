defmodule SymphonyElixir.StatusDashboard do
    ...
    if snapshot_data != state.last_snapshot_fingerprint or periodic_rerender_due?(state, now_ms) do
      content =
        format_snapshot_content(
          snapshot_data,
          tps,
          tps_graph(token_samples, now_ms, current_tokens)
        )
  ...
  defp format_snapshot_content(snapshot_data, tps, tps_graph_display \\ "", terminal_columns_override \\ nil) do
    case snapshot_data do
      {:ok, %{running: running, retrying: retrying, codex_totals: codex_totals} = snapshot} ->
        ...
        ([
           ...
           colorize("│ Throughput graph: ", @ansi_bold) <> colorize(tps_graph_display, @ansi_cyan),
  ...
  defp tps_graph(samples, now_ms, _current_tokens) do
