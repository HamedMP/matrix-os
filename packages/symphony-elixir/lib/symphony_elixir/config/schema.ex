defmodule SymphonyElixir.Config.Schema do
  ...
  alias SymphonyElixir.{Linear.Bridge, PathSafety}
      ...
      | api_key: resolve_secret_setting(settings.tracker.api_key, linear_api_key_fallback),
        ...
        assignee:
          resolve_secret_setting(
            ...
            first_present_env(["SYMPHONY_LINEAR_ASSIGNEE", "LINEAR_ASSIGNEE"])
          )
    }

    workspace = %{
      settings.workspace
      | root: resolve_path_value(settings.workspace.root, matrix_workspace_root_default())
    }
  ...
  defp resolve_text_value(value, fallback) when is_binary(value) do
    case resolve_env_value(value, fallback) do
      resolved when is_binary(resolved) and resolved != "" -> resolved
      _ -> fallback
    end
  end
  ...
  defp matrix_workspace_root_default do
    case System.get_env("SYMPHONY_WORKSPACE_ROOT") do
      value when is_binary(value) and value != "" ->
        value

      _ ->
        matrix_home = System.get_env("MATRIX_HOME") || "/home/matrix/home"
        Path.join([matrix_home, "projects", "matrix-os", "symphony-workspaces"])
    end
  end

  defp matrix_linear_bridge_credential, do: Bridge.credential()
