defmodule SymphonyElixir.Workspace do
  ...
  def remove(workspace) do
    try do
      ...
      error in [ArgumentError, ErlangError, File.Error] ->
        Logger.error("Workspace removal failed path=#{workspace} error=#{Exception.message(error)}")
        {:error, error, ""}
    end
  end
  ...
  def remove_issue_workspaces(identifier) when is_binary(identifier) do
    try do
      ...
      error in [ArgumentError, ErlangError, File.Error] ->
        Logger.error("Workspace issue cleanup failed identifier=#{identifier} error=#{Exception.message(error)}")
        :ok
    end
  ...
  def run_before_run_hook(workspace, issue_or_identifier) when is_binary(workspace) do
    try do
      ...
      error in [ArgumentError, ErlangError, File.Error] ->
        Logger.error("Workspace before_run hook setup failed path=#{workspace} error=#{Exception.message(error)}")
        {:error, error}
    end
  end
  ...
  def run_after_run_hook(workspace, issue_or_identifier) when is_binary(workspace) do
    try do
      ...
      error in [ArgumentError, ErlangError, File.Error] ->
        Logger.error("Workspace after_run hook setup failed path=#{workspace} error=#{Exception.message(error)}")
        :ok
    end
  end
