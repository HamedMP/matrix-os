defmodule SymphonyElixir.HttpServer do
  ...
  def start_link(opts \\ []) do
    with {:ok, %{host: host, port: port}} <- server_options(opts) do
      case port do
  ...
  defp server_options(opts) do
    port = Keyword.get_lazy(opts, :port, &Config.server_port/0)
    ...
    case Keyword.fetch(opts, :host) do
      ...
      :error ->
        case Config.settings() do
          {:ok, settings} -> {:ok, %{host: settings.server.host, port: port}}
