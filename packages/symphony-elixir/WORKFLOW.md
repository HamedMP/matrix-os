...
hooks:
  after_create: |
    git clone git@github.com:HamedMP/matrix-os.git .
  before_remove: |
...
codex:
  ...
  # Default sandbox comes from config/schema.ex: workspace-write with writes
  # limited to the issue workspace. Host read-only access is intentional so
  # agents can inspect installed toolchains and dependencies; /tmp is excluded
  # from automatic write access.
server:
  host: "127.0.0.1"
---
