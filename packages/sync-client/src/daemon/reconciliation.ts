...
import type { LocalFileState, SyncState } from "./types.js";
...
async function publishDownloadedConflictPath(
  ...
  for (let attempt = 1; attempt <= 1_000; attempt++) {
    ...
    try {
      await validateTarget?.(absolutePath);
...
export interface RemoteFileReconcileInput {
  syncRoot: string;
  localRel: string;
  remotePath: string;
  remoteHash: string;
  ...
  inspectLocal?: (localPath: string) => Promise<{
    hash: string;
    size: number;
    mtime: number;
  } | null>;
  validateConflictTarget?: (absolutePath: string) => Promise<void>;
}
...
export async function reconcileRemoteFileChange(
  ...
  const inspected = input.inspectLocal
    ? await input.inspectLocal(localPath)
    : await (async () => {
        try {
          const [hash, fileStat] = await Promise.all([hashFile(localPath), stat(localPath)]);
          return { hash, size: fileStat.size, mtime: Math.trunc(fileStat.mtimeMs) };
        } catch (err: unknown) {
          if (!isENOENT(err)) throw err;
          return null;
        }
      })();
  const localHash = inspected?.hash ?? null;
  const localSize = inspected?.size ?? 0;
  const localMtime = inspected?.mtime ?? 0;
  ...
  const updateDownloadedState = async (
    ...
    syncState.files[remotePath] = {
      ...
      mtime: Math.trunc(downloadedStat.mtimeMs),
...
export interface RemoteDeleteReconcileInput {
  syncRoot: string;
  localRel: string;
  remotePath: string;
  remoteHash: string;
  ...
  toRemotePath?: (localRel: string) => string;
  date?: Date;
  inspectLocal?: RemoteFileReconcileInput["inspectLocal"];
}
