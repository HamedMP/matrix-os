...
export interface TerminalRuntimeSocketStream {
  send(frame: z.input<typeof TerminalClientFrameSchema>): void;
  close(): void;
}
...
export class TerminalRuntimeSocketClient {
  ...
  attach(input: {
    ...
    mode: "hard" | "soft";
    size: { cols: number; rows: number };
    onFrame(frame: z.infer<typeof TerminalServerFrameSchema>): void;
    onClose(): void;
    ...
    socket.on("data", (chunk) => {
      try {
        for (const raw of decoder.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk))) {
          input.onFrame(TerminalServerFrameSchema.parse(raw));
        }
      } catch (error) {
    ...
    return {
      send: (frame) => {
        ...
        socket.write(encodeSocketFrame(TerminalClientFrameSchema.parse(frame)));
      },
      close,
    };
  }
