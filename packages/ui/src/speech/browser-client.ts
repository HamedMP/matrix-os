...
export class BrowserSpeechClientError extends Error {
  constructor(readonly code: BrowserSpeechErrorCode, readonly safeMessage: string) {
...
export interface BrowserSpeechClient {
  ...
  transcribe(input: { requestId: string; recording: Blob; signal: AbortSignal }): Promise<SpeechTranscriptionResponse>;
...
function requestUrl(baseUrl: string, path: string, runtimeSlot: string | undefined): string {
  const url = new URL(`${baseUrl.replace(/\/$/, "")}/api/speech${path}`);
  if (runtimeSlot && runtimeSlot !== "primary") {
    if (!/^[a-z0-9][a-z0-9_-]{0,79}$/.test(runtimeSlot)) {
      throw new BrowserSpeechClientError("unavailable", "Speech is unavailable");
    }
...
export function createBrowserSpeechClient(options: {
  ...
  async function request<T>(input: {
    ...
    try {
      response = await fetcher(requestUrl(options.baseUrl, input.path, options.runtimeSlot), {
        method: input.method,
        body: input.body,
        signal,
        redirect: "error",
  ...
  return {
    ...
    transcribe(input) {
      ...
      form.set("requestId", SpeechRequestIdSchema.parse(input.requestId));
