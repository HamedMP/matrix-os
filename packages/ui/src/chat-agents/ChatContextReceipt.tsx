...
export function ChatContextReceipt({ context }: { context?: ChatRunContext }) {
  ...
  return <div className="my-2 grid gap-2 text-xs" style={{ color: "var(--text-secondary, var(--matrix-muted-fg))" }}>
    {context.agent ? <p className="font-medium">{context.agent.name} · Hermes</p> : null}
    {context.chats.length ? <details className="rounded-lg border px-3 py-2">
