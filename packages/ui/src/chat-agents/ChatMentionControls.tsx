...
export function useChatMentionPermission(
  scope: string,
  resources: CanonicalChatResourceReference[],
  permissionMode: string,
  requestIdentity?: string | number,
) {
  const agent = resources.find((resource) => resource.kind === "agent");
  const agentId = agent?.id;
  // Hosts keep requestIdentity stable while typing and rotate it when replacing a draft.
  const key = JSON.stringify([scope, agentId, agent?.revision, requestIdentity]);
  const [confirmation, setConfirmation] = useState<{ key: string; allowed: boolean } | null>(null);
  // Forget consent immediately when the request, selected Agent revision, or Chat changes.
