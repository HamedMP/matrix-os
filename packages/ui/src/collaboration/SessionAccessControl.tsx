import { CollaborationMemberSchema, CollaborationProjectAccessReadinessSchema, CollaborationReadinessSchema, CollaborationScopeSchema, type CollaborationProjectAccessReadiness, type CollaborationReadiness } from "@matrix-os/contracts";
...
export function SessionAccessControl({ api, scope, zIndex }: {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [readinessResult, projectResult] = await Promise.allSettled([
        scope.kind === "project" || scope.kind === "chat"
          ? api.post(`/api/collaboration/scopes/${encodeURIComponent(scope.id)}/policy/preflight`, {})
