...
function renderMatrixMark(className = 'matrix-mark'): string {
  return `<span class="${className}" aria-hidden="true">M</span>`;
}

function renderFeatureShowcase(): string {
  return `<section class="showcase" data-matrix-feature-showcase="product">
      <div class="wordmark">${renderMatrixMark()}<span>matrix-os</span></div>
      ...
      <div class="workspace" aria-label="Matrix agent workspace preview">
        ...
        <div class="workspace-grid">
          ...
          <div class="agents"><p class="agents-label">Agents</p><div class="agent-list"><span><i></i>Claude · running</span><span><i></i>Codex · PR opened</span><span class="agent-idle"><i></i>Hermes · idle</span></div></div>
        </div>
      </div>
      <div class="mobile-agents" aria-label="Supported agents">
        <span>Claude</span><span>Codex</span><span>Cursor</span><span>Hermes</span>
      </div>
    </section>`;
}
  ...
  return `<aside class="panel">
      ...
      <div class="card-shell">
        <section class="card" id="handoff-card" aria-live="polite">
          ${renderMatrixMark('matrix-mark status-mark')}
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .matrix-mark { width: 34px; height: 34px; display: inline-grid; flex: none; place-items: center; border: 1px solid ${c.border}; border-radius: 8px; background: ${c.card}; color: ${c.forest}; font-family: ${fonts.display}; font-size: 1.25rem; line-height: 1; }
    ...
    .card-shell { overflow: hidden; padding: 1rem; border: 1px solid ${c.border}; border-radius: 1rem; background: ${c.card}; box-shadow: ${cardShadow}; }
    ...
    .status-mark { width: 56px; height: 56px; margin-bottom: 1.75rem; border-radius: 14px; font-size: 2rem; }
    ...
    .card.failed .spinner, .card.failed .status-mark { display: none; }
    ...
    @media (max-width: 900px) {
      ...
      .mobile-agents { display: grid; max-width: 28rem; margin: 1.75rem auto 0; border-top: 1px solid ${c.border}; border-bottom: 1px solid ${c.border}; }
      .mobile-agents span { padding: 0.75rem 0; border-top: 1px solid ${c.border}; color: ${c.forest}; font-size: 0.875rem; font-weight: 500; }
      .mobile-agents span:first-child { border-top: 0; }
