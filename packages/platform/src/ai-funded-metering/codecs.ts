/**
 * Funded-AI metering schemas and pure helpers.
 *
...
export const TOKEN_PATTERN = /^sk-matrix-funded-([A-Za-z0-9][A-Za-z0-9_.:-]{0,79})\.([A-Za-z0-9_-]{43})$/;
export const ReferenceSchema = z.string().min(1).max(160).regex(/^[A-Za-z0-9][A-Za-z0-9_.:-]*$/);
...
export const ModelIdsSchema = z.array(z.string().min(3).max(200)).max(64);
