...
export interface FundedModelProbeService { probe(modelId: string): Promise<FundedModelProbeResult> }
...
async function readCoarseReady(response: Response): Promise<boolean> {
  ...
  try {
    while (true) {
      ...
      if (size > MAX_PROBE_RESPONSE_BYTES) { await reader.cancel(); return false; }
      chunks.push(next.value);
    }
  } finally { reader.releaseLock(); }
  try {
    const value: unknown = JSON.parse(new TextDecoder().decode(Buffer.concat(chunks)));
    return !!value && typeof value === "object" && "ready" in value && value.ready === true;
  } catch (error) {
...
export function createFundedModelProbeService(input: {
  ...
  const inFlight = new Map<FundedProbeModel, Promise<FundedModelProbeResult>>();
  ...
  return {
    async probe(modelId) {
      ...
      const task = (async (): Promise<FundedModelProbeResult> => {
        const admitted = await reserveFundedModelProbe({ db: input.db,
        ...
        try {
          ...
          ready = await readCoarseReady(response);
        } catch (error) {
        ...
        const result = { ready, checkedAt: new Date(checked).toISOString(),
          staleAfter: new Date(checked + (ready ? POSITIVE_TTL_MS : NEGATIVE_TTL_MS)).toISOString() };
        cache.set(model, result);
        return result;
