...
export class CanvasService {
  ...
  private readonly resolveProjectWorkingDirectory?: (ownerId: string, projectId: string) => Promise<string | null>;
  ...
  constructor(private readonly repository: CanvasRepository, options: CanvasServiceOptions = {}) {
    ...
    this.resolveProjectWorkingDirectory = options.resolveProjectWorkingDirectory;
  ...
  private async createTerminal(record: CanvasRecord, action: Extract<CanvasAction, { type: "terminal.create" }>) {
    ...
    let safeCwd = this.homePath ? resolveWithinHome(this.homePath, cwd) : cwd;
    if (!safeCwd) throw new CanvasNotFoundError("cwd");
    if (scopedProjectId) {
      if (!this.homePath || !this.resolveProjectWorkingDirectory) {
        throw new CanvasConfigurationError("project working-directory resolver is required");
      }
      const projectPath = await this.resolveProjectWorkingDirectory(record.ownerId, scopedProjectId);
      if (!projectPath) throw new CanvasNotFoundError(scopedProjectId);
      try {
        const [canonicalProjectPath, canonicalCwd, cwdStats] = await Promise.all([
          realpath(projectPath),
          realpath(safeCwd),
          stat(safeCwd),
        ]);
        const relativeCwd = relative(canonicalProjectPath, canonicalCwd);
        if (
          ...
          throw new CanvasNotFoundError("cwd");
        }
        safeCwd = canonicalCwd;
      } catch (error: unknown) {
        if (error instanceof CanvasNotFoundError) throw error;
