...
export function createProjectTransitionJournal(options: {
  ...
  async function replayPreparation(input: {
    scopeId: string;
    requestedBy: string;
    ...
    try {
      const scope = await options.db.selectFrom("collaboration_scopes")
        ...
        .executeTakeFirst();
      if (!scope || scope.kind !== "project" || scope.owner_id !== parsed.requestedBy) {
        throw new ProjectTransitionError("conflict");
      }
      const replay = await options.db.selectFrom("collaboration_operations")
      ...
      const existing = await options.db.selectFrom("collaboration_transitions").selectAll()
      ...
      console.warn("[collaboration-project] transition replay failed", error instanceof Error ? error.name : "UnknownError");
