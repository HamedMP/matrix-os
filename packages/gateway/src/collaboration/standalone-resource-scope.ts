...
export class StandaloneResourceScopeService {
  ...
  private async current(input: ShareInput, entry: CatalogEntryRecord): Promise<string> {
    ...
    if (entry.kind === "app") {
      if (!this.options.resources.resolveAppIncarnation) throw new ResourceCatalogError("unavailable");
      const incarnation = await this.options.resources.resolveAppIncarnation({ ownerId: input.ownerId,
        projectId: entry.projectId, appId: entry.path });
