...
export function createProjectTransitionCoordinator(options: {
  ...
  const maxActiveTransitions = options.maxActiveTransitions ?? MAX_ACTIVE_TRANSITIONS;
  if (!Number.isInteger(maxActiveTransitions)
    || maxActiveTransitions < 1 || maxActiveTransitions > MAX_ACTIVE_TRANSITIONS) {
    throw new RangeError("Invalid active project transition limit");
  }
  const active = new Map<string, Promise<void>>();
  let drainActive: Promise<void> | undefined;
  let closing = false;
  ...
  async function recoverFailedTransition(transitionId: string): Promise<void> {
    await options.transitions.recoverTransition(transitionId, {
      cleanupStaging: async (input) => cleanupStaging(input),
      completePublication: async ({ signal }) => {
        if (signal.aborted) throw signal.reason;
      },
  ...
  async function drainPrepared(): Promise<void> {
    if (closing || active.size >= maxActiveTransitions) return;
    const rows = await options.db.selectFrom("collaboration_transitions")
      .select("id")
      .where("status", "=", "prepared")
      .orderBy("created_at", "asc")
      .limit(maxActiveTransitions - active.size)
      .execute();
    for (const row of rows) schedule(row.id);
  }

  function requestDrain(): void {
    if (closing || drainActive) return;
    const operation = drainPrepared().catch((error: unknown) => {
      console.warn(
        "[collaboration-project] durable transition drain deferred",
        error instanceof Error ? error.name : "UnknownError",
      );
    }).finally(() => {
      if (drainActive === operation) drainActive = undefined;
    });
    drainActive = operation;
  }

  async function recover() {
    const result = await recoverIncomplete();
    requestDrain();
    return result;
  }
  ...
  function schedule(rawTransitionId: string): boolean {
    ...
    if (active.size >= maxActiveTransitions) return true;
    const operation = run(transitionId)
      .then(() => undefined)
      .catch(async (error: unknown) => {
        console.warn(
          "[collaboration-project] transition execution failed",
          error instanceof Error ? error.name : "UnknownError",
        );
        await recoverFailedTransition(transitionId);
      })
      .finally(() => {
  ...
  async function shutdown(): Promise<void> {
    if (closing) return;
    closing = true;
    if (drainActive) await drainActive;
