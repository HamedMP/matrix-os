...
export function createTerminalWorkspaceRoutes(options: { runtime: TerminalWorkspaceRouteRuntime; homePath?: string }): Hono {
  const app = new Hono();
  ...
  app.post("/workspaces/:workspaceId/tabs", mutationLimit, async (c) => {
    try {
      ...
      return c.json({ tab: await options.runtime.createTab(workspaceId, body) }, 201);
    } catch (error) { return requestFailure(c, error); }
  });
