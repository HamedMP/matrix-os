...
export class ChatAgentStore {
  ...
  // At most two open directory cursors; resume bounded batches instead of rescanning a prefix.
  private sweepRoot: Dir | null = null;
  private sweepOwner: { name: string; directory: Dir } | null = null;
  ...
  async close(): Promise<void> {
    ...
    await this.sweep;
    await this.closeSweepDirectories();
  }
  ...
  private async read(path: string): Promise<StoredAgent | null> {
    let file;
    try { file = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK); } catch (error: unknown) {
      if (isCode(error, "ENOENT")) return null;
  ...
  private async closeSweepDirectories(): Promise<void> {
    const directories = [this.sweepOwner?.directory, this.sweepRoot];
    this.sweepOwner = null;
    this.sweepRoot = null;
  ...
  private async sweepTemporaryFiles(): Promise<void> {
    try {
      ...
      if (!root) { await this.closeSweepDirectories(); return; }
      this.sweepRoot ??= await opendir(root);
      let owners = 0;
      let files = 0;
      while (owners < 256 && files < MAX_AGENTS + 128) {
        if (!this.sweepOwner) {
          const owner = await this.sweepRoot.read();
          if (!owner) { await this.closeSweepDirectories(); return; }
          owners++;
          if (!owner.isDirectory() || !/^[a-f0-9]{64}$/.test(owner.name)) continue;
          const directory = await this.directory([owner.name]);
          if (!directory) continue;
          this.sweepOwner = { name: owner.name, directory: await opendir(directory) };
        }
        const current = this.sweepOwner;
        // Revalidate persisted cursors before accessing paths, including across timer ticks.
        const directory = await this.directory([current.name]);
        if (!directory) {
          this.sweepOwner = null;
