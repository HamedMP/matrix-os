...
- **Production topology**: customer runtime is per-user VPS with host services and local Postgres, not the legacy shared user-container path
- **No emojis** in code or docs unless explicitly requested
- **Kysely/PostgreSQL** for all database access (never add SQLite, Drizzle ORM, or better-sqlite3)
...
packages/platform/   # Control plane: Clerk auth, VPS provisioning/routing, integrations
...
distro/              # customer VPS, cloudflared, systemd, and local dev configs
```
