...
3. Run `matrix agent auth scan` and ask which found credentials I want copied
4. Transfer approved credentials with `matrix upload --secret`
5. Start my preferred coding agent on Matrix with `matrix run`
6. Explain the reconnect and recovery commands only after the main path works

Then proceed step by step. Keep the happy path small:
...
## Step 3: Configure and run agents inside Matrix
...
If the scan finds local credential files and I approve copying them, use only the matching Matrix CLI transfer command. Do not read or print the file contents:
...
If Claude Code is available only through macOS Keychain, do not extract it. Launch Claude on Matrix and let me complete remote login.
...
Use the selected agent only; do not launch multiple agents unless I explicitly ask.
