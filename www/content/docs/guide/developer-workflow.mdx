...
4. Start my preferred coding agent on Matrix with `matrix run`
5. Let me complete that tool's browser/device login inside the remote terminal
...
Use browser/device login inside the Matrix VPS. Do not scan the local machine for credential files, and do not transfer local secret files as part of onboarding.
...
For a coding agent, run one command at a time:
