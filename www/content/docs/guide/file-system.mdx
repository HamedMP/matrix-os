...
~/matrixos/
  ...
  +-- groups/            # Shared group state (spec 062)
  |     +-- {slug}/
  |           +-- manifest.json       # Group metadata and room_id
  |           +-- members.cache.json  # Observed members + power levels
  |           +-- apps/{app}/         # Shared app code
  |           +-- data/{app}/
  |           |     +-- state.bin     # Yjs CRDT snapshot
  |           |     +-- log.jsonl     # Op log
  |           |     +-- queue.jsonl   # Offline queue
  |           |     +-- last_sync.json
  |           +-- acl/{app}.json      # Per-app ACL cache
