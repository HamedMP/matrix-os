...
~/matrixos/
  ...
  |
  +-- .agents/
  |     +-- skills/      # Agent Skills directories (matrix-app-builder/SKILL.md, ...)
  |
  +-- agents/
...
Markdown previews render referenced SVG image assets inline. Local SVG paths are resolved through the same path-sandboxed `/files/*` serving route, while unsafe or broken SVG references degrade to a readable placeholder instead of interrupting the document.
...
While most state is file-based, PostgreSQL managed through Kysely handles structured durable data:
...
PostgreSQL provides concurrent access, transactions, and recovery for structured kernel and app state.
