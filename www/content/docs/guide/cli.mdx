...
matrix download --secret .codex/auth.json ~/.codex/auth.json
```

For credential files, add `--secret` so transferred files are written owner-only.
...
Agents should run the Matrix CLI command and must not print credential file contents.

## Agent auth scan

`matrix agent auth scan` checks for local AI-agent credential files without reading or printing tokens.

```bash
matrix agent auth scan
matrix agent auth scan --json
```

The scan can detect transfer-ready files for Codex, Claude Code file credentials, OpenCode, and Pi. macOS Keychain-only Claude Code credentials are reported as manual because they cannot be moved through generic file upload.
