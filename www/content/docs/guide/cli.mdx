...
`matrix ssh` connects directly to your customer VPS as the `matrix` user and auto-attaches the default zellij session, `matrix-main`. If zellij is unavailable, it falls back to tmux with the same session name.

```bash
matrix ssh                  # your own instance
matrix ssh @hamed           # your own handle; shared-handle SSH is not enabled yet
matrix ssh --raw            # plain SSH login shell, no session auto-attach
```

<Callout type="info">
  SSH currently resolves your active profile's own VPS. The old bastion path is deferred until a production bastion exists.
</Callout>
