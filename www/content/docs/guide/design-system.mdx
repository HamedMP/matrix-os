...
| Variable | Default | Usage |
|----------|---------|-------|
| `--matrix-font-sans` | `"Instrument Sans", system-ui, sans-serif` | Body text |
...
- **Background**: Cream `#f6f4ed` -- the canvas around navigation and secondary surfaces
- **Primary**: Forest `#434e3f` -- the stable brand color for primary UI and emphasis
- **Accent**: Ember `#d06f25` -- the signature accent, used for active states, links, and focus rings
...
- **Border**: Warm stone `#d6d3c8` -- barely visible structure

### Surface Hierarchy

Three levels of elevation:

1. **Background** (`--matrix-bg`): The lowest surface. Cream canvas.
...
The shell uses Instrument Sans for UI text and JetBrains Mono for code. Apps should inherit via `var(--matrix-font-sans)` rather than redeclaring fonts.
...
- **Skill**: `~/.agents/skills/matrix-design-system/SKILL.md` -- Matrix theme variables, shadcn-style controls, iframe-safe layouts, icon rules
