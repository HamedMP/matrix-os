...
Native app sessions run as the non-root Matrix runtime user on your VPS. The gateway owns the session, enforces owner checks, caps active sessions, cleans up stale sessions, and proxies the xpra stream through a same-origin Matrix URL.
