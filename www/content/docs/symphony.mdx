...
Symphony turns selected Linear or Matrix-native tickets into Matrix-owned
coding-agent runs. You choose which team, labels, states, and assignees Symphony
can react to. Matrix creates the worktree, starts the cloud agent session, and
shows the status in one dashboard.
...
poll Linear and claim work, or assign a Matrix-native ticket from Workspace.
...
Matrix Desktop uses the same Symphony routes. The desktop app can show run
status, open the related Workspace context, and observe or take over the cloud
session, but it does not start a local coding agent.
...
Project owners can share ticket boards with authorized Matrix users. Shared
board permission controls whether a teammate can read tickets or claim work with
their Symphony runner. Removing a teammate blocks future ticket and run control
without revealing private ticket, credential, repository, or filesystem details.
