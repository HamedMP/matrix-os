...
In the browser shell, Canvas remains the primary workspace. The built-in Workspace view shows current-project coding-agent thread and preview summaries from the same gateway runtime summary. It is read-only: use desktop or mobile for thread details, approvals, review editing, and source-control actions.
