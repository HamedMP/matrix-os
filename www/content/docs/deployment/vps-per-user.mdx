...
| `MATRIX_HOST_BUNDLE_URL` | No | Optional override for the exact bundle URL. CI publishes the default bundle and sidecar `.sha256` to R2. |
| `MATRIX_HOST_BUNDLE_BASE_URL` | No | Optional public base URL for default bundle URL generation when not using `MATRIX_HOST_BUNDLE_URL`. |
