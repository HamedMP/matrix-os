...
- `system/db/snapshots/<timestamp>.sql.gz`: compressed Postgres custom-format snapshot.

Retention is intentionally conservative in this slice. `matrixctl r2 prune system/db/snapshots/` is bounded and safe to no-op until richer retention metadata is added.
