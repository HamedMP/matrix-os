...
Messages brings Telegram and WhatsApp into Matrix OS through a private Matrix backbone running on your VPS. Conversations appear in the Messages app, and [Hermes](/docs/hermes) starts with no access to any room until you grant it.
...
<Cards>
  ...
  <Card title="WhatsApp" description="Connects by scanning a QR code. May require relinking if the paired-device session expires or a stale VPS restore is detected." />
</Cards>
...
See [Hermes](/docs/hermes) for credential requirements and how Hermes is enabled.

<Callout type="warn" title="Hermes needs Claude credentials">
  Hermes uses Claude to generate summaries, drafts, and automation responses. Without a valid Claude credential on your VPS, it cannot act on any room — even if room-level permissions are enabled. See [Hermes](/docs/hermes) for setup.
</Callout>
...
Messaging state is part of the owner-controlled VPS backup lifecycle. Telegram and Matrix conversation mappings restore with the backup. WhatsApp may require relinking after a stale restore or when the paired-device session is no longer recognized.
