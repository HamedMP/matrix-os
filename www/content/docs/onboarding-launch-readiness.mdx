...
Launch-critical product emails route through Loops. The template set covers account welcome, auth-product prompts when Loops owns them, paid beta activation, billing success handoff, payment failure, subscription changes, subscription cancellation, CLI activation, cloud-coding activation, and internal launch alerts.

The detailed planning artifact is `specs/082-paid-beta-readiness/launch-email-templates.md`. It defines the Loops event/template names, required contact properties, Clerk and Stripe event mappings, app-triggered Matrix events, idempotency expectations, non-blocking failure behavior, and read-only Loops CLI validation commands. Clerk remains responsible for auth mechanics, and Stripe or Clerk Billing remains responsible for receipts and payment collection.
