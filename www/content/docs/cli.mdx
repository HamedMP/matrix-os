...
`matrix run` starts a command on your Matrix computer and, with `-it`, attaches an interactive terminal to it over the gateway WebSocket.
...
Shell sessions on your Matrix computer are backed by zellij. The same sessions visible in the web shell are available from the CLI.
...
If `matrix run -it`, `matrix shell new`, or `matrix shell attach` returns a `zellij_failed` error, run `matrix shell ls` and connect to an existing session rather than retrying the same create path.
</Callout>
