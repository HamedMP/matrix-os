...
The web shell at `https://app.matrix-os.com` is the primary interface to your Matrix computer. It gives you Terminal, Files, Canvas, apps, integrations, and settings — all running on your persistent VPS.

## Terminal and sessions

The Terminal is a persistent, zellij-backed shell that keeps running after you close the browser. Sessions are shared across the web shell, the CLI, and the desktop app through the same `/api/terminal/sessions` endpoint. Starting work in the browser and reconnecting from the CLI later picks up exactly where you left off.
