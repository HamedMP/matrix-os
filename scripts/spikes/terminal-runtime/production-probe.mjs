...
if (operation === 'create' || operation === 'create-race' || operation === 'create-agent') {
  ...
  roleStage = 'classify'; const zellij = processes.filter((entry) => entry.comm === 'zellij' && !entry.args.includes('list-sessions')).sort((left, right) => left.pid - right.pid);
  const keeper = processes.find((entry) => entry.args.some((argument) => argument.endsWith('/keeper.js')));
  const workloads = processes.filter((entry) => entry.pid !== keeper?.pid && !zellij.some((process) => process.pid === entry.pid) && !(entry.comm === 'zellij' && entry.args.includes('list-sessions')));
