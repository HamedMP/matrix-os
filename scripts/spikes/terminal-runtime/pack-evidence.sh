...
if [ ! -f "$evidence_root/summary.json" ] || [ -L "$evidence_root/summary.json" ]; then
  ...
  read -r keeper_stage keeper_responsive keeper_zellij keeper_shell keeper_agent < <(/opt/matrix/runtime/node/bin/node -e '
    ...
    if(!/^(descriptor|launch|cgroup|readiness|notify)$/.test(v.stage)||typeof v.responsive!=="boolean"||
      !Number.isInteger(v.zellij)||v.zellij<0||v.zellij>999||typeof v.shell!=="boolean"||typeof v.agent!=="boolean")process.exit(1);
    process.stdout.write(`${v.stage} ${v.responsive?1:0} ${v.zellij} ${v.shell?1:0} ${v.agent?1:0}\n`);
  ' "/run/matrix-terminal-runtime-spikes/${run_namespace}/startup-stages/${base_id}.json" 2>/dev/null || printf 'unknown 0 0 0 0\n')
...
if [ "$summary_status" != pass_pass ]; then
  read -r failure_stage failure_code failure_responsive failure_zellij failure_shell failure_agent < <(/opt/matrix/runtime/node/bin/node -e '
    const fs=require("fs"),v=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));
    if(!/^(descriptor|launch|cgroup|readiness|notify)$/.test(v.stage)||!/^[a-z0-9_]{1,32}$/.test(v.code)||
      ...
      typeof v.shell!=="boolean"||typeof v.agent!=="boolean")process.exit(1);
    process.stdout.write(`${v.stage} ${v.code} ${v.responsive?1:0} ${v.zellij} ${v.shell?1:0} ${v.agent?1:0}\n`);
  ' "/run/matrix-terminal-runtime-spikes/${run_namespace}/startup-failures/${base_id}.json" 2>/dev/null || printf 'unknown unknown 0 0 0 0\n')
  echo "spike_pack_evidence_failed_${failure_stage}_${failure_code}_r${failure_responsive}_z${failure_zellij}_s${failure_shell}_a${failure_agent}"
  exit 0
fi
