...
IFS='/' read -r -a build_path_components <<<"$build_dir"
checked_build_dir="/"
for component in "${build_path_components[@]}"; do
  case "$component" in
    '' | '.')
      continue
      ;;
    '..')
      checked_build_dir="${checked_build_dir%/*}"
      if [ -z "$checked_build_dir" ]; then
        checked_build_dir="/"
      fi
      ;;
    *)
      checked_build_dir="${checked_build_dir%/}/$component"
      if [ -L "$checked_build_dir" ]; then
