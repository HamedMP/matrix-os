...
import { ActivityIndicator, AppState, Linking, Pressable, RefreshControl, ScrollView, Text, TextInput, View } from "react-native";
import { Ionicons } from "@expo/vector-icons";
import { Stack, useFocusEffect, useLocalSearchParams, useRouter } from "expo-router";
import { StyleSheet, useUnistyles } from "react-native-unistyles";
import { ReviewIdSchema, type FileBrowseResponse, type FileReadRequest, type FileReadResponse, type FileSearchResponse, type FileWriteRequest, type ReviewSnapshot, type ReviewSummary, type SourceControlCreatePullRequestRequest, type SourceControlCreatePullRequestResponse, type SourceControlPrepareCommitRequest } from "@matrix-os/contracts";
import { useGateway } from "@/app/_layout";
import { CODING_AGENTS_MOBILE_WORKSPACE } from "@/lib/feature-flags";
import { EmptyText, INITIAL_STATE, Section, canOpenExternalUrl, capabilityEnabled, type ScreenState } from "@/components/agents/agent-workspace-shared";
...
export default function ReviewsScreen() {
  ...
  // Refresh whenever the screen regains focus (returning from the composer) or
  // the app comes back to the foreground so stale review rows and snapshots do
  // not linger until a manual pull-to-refresh. The first focus coincides with
  // mount, which already loads, so skip it to avoid a duplicate request.
    ...
    >
      ...
      <View style={styles.header}>
        <View style={styles.headerIcon}>
          <Ionicons name="git-pull-request-outline" size={22} color={theme.colors.forest} />
        </View>
      ...
      {capabilityEnabled(summary, "codingAgentsReview") ? (
        ...
        <EmptyText>Reviews are not available for this runtime.</EmptyText>
      )}
    </ScrollView>
  );
}
