...
V1 grants one preset for the whole project. The per-folder, per-app, per-command and per-integration selectors in the table below are deferred; the table is retained as the target capability vocabulary. Resource roles are editable presets (Viewer, Discussant, Contributor, Maintainer), expanded into explicit capabilities. Grants name audience, resource selectors, actions, policy version, expiry and approval requirements. A share preview explains exactly what this recipient can do:
...
Every shared run executes in the OS-enforced isolation that the shipped scope-runtime already provides. A Git worktree is not a security sandbox. Filtered workspaces, publication brokers and Chat audience data ceilings belong to granular sharing and are deferred with it; in V1 the whole project is the audience for every Chat in it.
...
Release tests must prove the organization-only precondition on every path with no environment flag or cohort record consulted, fail-closed behavior on missing configuration, two-computer access through the relay with no platform authorization decision and no payload parsed or logged there, home rejection of forged/expired tickets delivered by the relay, revocation during partitions, multiple members using one owner source, default group Chat, member commit/push/PR through the broker under the owner identity with the requesting member audited, shared Codex and Claude runs on the project root, share-time inventory of every Chat root, sandbox escape denial, and Web Canvas / Web Desktop / Electron Desktop parity. Invite quotes, member-computer migration and managed Matrix group text are deferred and not release gates. Use real Postgres races and disposable enrolled VPSes; no claimed provider support without versioned live evidence. Include all applicable surfaces and a separate documentation PR in `FinnaAI/matrix-os-site/content/docs/`.
