...
## Frozen payload unions (S02)

S02 froze the following Zod 4 schemas in `packages/contracts` (all exported from `@matrix-os/contracts`; every object is `.strict()`, every string bounded, every identifier a safe reference). Later packets extend these modules; they do not add competing stores or parallel schemas.

| Module | Schema | Route rows / use |
| --- | --- | --- |
| `collaboration-direct.ts` | `COLLABORATION_DIRECT_PROTOCOL_VERSION` (`2`), `COLLABORATION_DIRECT_LIMITS` | Ticket/session/evidence TTLs, byte, page, connection and replay limits from "Ticket/proof and limits" |
| `collaboration-direct.ts` | `CollaborationConnectionTicketSchema`, `CollaborationSignedConnectionTicketSchema` | Issued by `POST /api/collaboration/connections`; binds actor, nonce, proof-key thumbprint, resource, purpose, logical runtime id and authority generation (never a hostname), max actions and a ≤30 s expiry |
| `collaboration-direct.ts` | `CollaborationDirectSessionRequestSchema`, `CollaborationDirectSessionRenewRequestSchema`, `CollaborationDirectSessionSchema` | `POST /api/collaboration/direct-sessions`, `/direct-sessions/:sessionId/renew`, `DELETE /direct-sessions/:sessionId`; session ≤5 min with a ≤20 s organization-evidence deadline and an https origin without a path |
| `collaboration-direct.ts` | `CollaborationDirectRequestSignatureSchema`, `CollaborationDirectHandshakeFrameSchema` | Every `D` request signature (method, canonical path, query, body digest, conditional headers, session, nonce) and the first bounded WebSocket frame |
| `collaboration-direct.ts` | `CollaborationRuntimeEndpointRegistrationSchema` | `POST /internal/collaboration/runtime-endpoints`: relay handle from customer-VPS enrollment (never a URL), ed25519 keys, optional future direct origin restricted to public hostnames |
| `collaboration-direct.ts` | `CollaborationResourceDirectoryEntrySchema` | `GET /api/collaboration/shared`, `/inbox`, `PUT /internal/collaboration/directory`; metadata only with the caller's pending/active/declined state |
| `collaboration-direct.ts` | `CollaborationControlAssertionSchema`, `CollaborationDenialSchema`, `CollaborationControlAckSchema` | `/internal/collaboration/control` (WS), `/internal/organizations/access/resolve`, `/internal/collaboration/control/ack`; fixed-expiry membership assertions anchored to request start, denials pending until acknowledged, monotonic generations |
| `collaboration-direct.ts` | `COLLABORATION_DIRECT_ROUTES` | The exact V1 method/path allowlist with authority (`platform` or `home`), auth class, body limit and referenced schemas; no catch-all, no deferred routes |
| `collaboration-capabilities.ts` | `CollaborationPresetSchema` (`viewer`, `contributor`), `CollaborationCapabilityActionSchema`, `COLLABORATION_PRESET_CAPABILITIES`, `expandCollaborationPreset` | Preset expansion used by the authority evaluator and every renderer |
| `collaboration-capabilities.ts` | `CollaborationResourceKindSchema`, `CollaborationAudienceSchema`, `CollaborationGrantSchema`, `CollaborationGrantActivationSchema`, `CollaborationCreateGrantRequestSchema`, `CollaborationPatchGrantRequestSchema` | `GET/POST /scopes/:scopeId/grants`, `PATCH/DELETE /grants/:grantId`, `POST /invitations/:id/accept` and `/decline` (activation rows, `active` or `declined`) |
| `collaboration-capabilities.ts` | `CollaborationEffectiveAccessSchema`, `CollaborationReadinessSchema`, `readinessItemsForResourceKind` | `GET /scopes/:scopeId/access`, `POST /scopes/:scopeId/policy/preflight`; readiness items exist only for projects and Chats, and a `ready` project or Chat must name its source kind and effective submit mode |
| `collaboration-capabilities.ts` | `CollaborationDirectErrorSchema` | Every error response; generic codes and provider-free text |
| `collaboration-execution.ts` | `CollaborationExecutionPolicySchema`, `CollaborationExecutionPolicyPutRequestSchema`, `resolveCollaborationEffectiveSubmitMode` | `GET/PUT /scopes/:scopeId/execution-policy`; one policy per project or standalone Chat scope, effective mode `members` only when organization metadata is `members` and the policy follows the organization |
| `collaboration-execution.ts` | `CollaborationRunSubmitRequestSchema`, `CollaborationRunQueueSchema`, `CollaborationRunSchema`, `CollaborationRunBindingSchema` | `GET/POST /scopes/:scopeId/chat/requests`; requests select harness, model and root only; the binding is immutable and status-free; `interrupted` and the deciding actor live on the run |
| `collaboration-execution.ts` | `CollaborationRunCancelRequestSchema`, `CollaborationRunRetryRequestSchema`, `CollaborationToolApprovalDecisionRequestSchema`, `COLLABORATION_RUN_CONTROL_ACTORS`, `collaborationRunControlPermitted` | `/chat/requests/:requestId/cancel` and `/retry`, `/chat/approvals/:approvalId/decision` (identity in the path; the body pins `runId` as the existing handler and CLI do); cancel and approval for requester or scope owner, retry for the requester only |
| `collaboration-execution.ts` | `CollaborationGitActionRequestSchema`, `CollaborationGitOperationSchema` | `GET /scopes/:scopeId/project/git`, `POST /project/git/actions`; status, diff, commit, push, pr with no approval, force or remote-change field; branch names follow the gateway's git-check-ref-format rules (`isCollaborationGitBranchName` mirrors `isValidGitBranchName`) |
| `collaboration-peer.ts` (`@deferred`) | `CollaborationPeerOperationTicketSchema`, `CollaborationPeerSessionRequestSchema`, `CollaborationPeerChunkManifestSchema` | Reserved for S11/S13; no V1 route |
| `organization-billing.ts` (`@deferred`) | `CollaborationRunPayerBindingSchema`, `OrganizationInviteQuoteSchema`, `OrganizationMemberComputerAssignmentSchema` | Reserved for S14; V1 runs are owner-funded |

Version negotiation: every ticket, session, signed request, handshake frame, registration and control frame carries `protocolVersion: 2`. A home or platform that receives another version rejects it with the `upgrade_required` error code before any authorization work; there is no downgrade path and old clients receive upgrade-required rather than a fallback session. The value is raised only by a coordinated release that ships clients and homes together.
