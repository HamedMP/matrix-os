...
| POST `/scopes/:scopeId/invitations`; GET `/api/collaboration/invitations/:id`; POST `/:id/accept` or `/decline` | D manage / exact invitee who is a current member of the scope's organization; identifiers outside that organization are not resolved; for an organization-wide share, opening the pending item is the accept: the open performs one atomic pending-to-active transition for that member as a single conditional write (`WHERE state = 'pending'` on the member's activation row), records the activating actor and time, and grants nothing to a member whose row is still pending; a concurrent second open observes the already-active row and is idempotent; pending-invitation T permits only these actions |
