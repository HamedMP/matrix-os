...
| `collaboration-capabilities.ts` | `CollaborationEffectiveAccessSchema`, `CollaborationReadinessSchema`, `readinessItemsForResourceKind` | `GET /scopes/:scopeId/access`, `POST /scopes/:scopeId/policy/preflight`; readiness items exist only for projects and Chats |
...
| `collaboration-execution.ts` | `CollaborationRunCancelRequestSchema`, `CollaborationRunRetryRequestSchema`, `CollaborationToolApprovalDecisionRequestSchema`, `COLLABORATION_RUN_CONTROL_ACTORS`, `collaborationRunControlPermitted` | `/chat/requests/:requestId/cancel` and `/retry`, `/chat/approvals/:approvalId/decision`; cancel and approval for requester or scope owner, retry for the requester only |
| `collaboration-execution.ts` | `CollaborationGitActionRequestSchema`, `CollaborationGitOperationSchema` | `GET /scopes/:scopeId/project/git`, `POST /project/git/actions`; status, diff, commit, push, pr with no approval, force or remote-change field |
