...
- Gateway child review round (#1819, 2026-09-21): Greptile P2 "requests are not batched" fixed by micro-batching concurrent distinct-actor lookups into one platform request of at most 100 actors, each lookup settling individually (RED: new batching test expected 2 requests for 130 actors, got 130; GREEN: `organization-membership-client` 6/6). P2 "test fixture violates contract" fixed by returning the full `OrganizationMembershipAssertion` shape (`aiSubmission`, `membershipEpoch`) from the precondition fixture. Focused suites 29/29; typecheck and pattern scan clean.
