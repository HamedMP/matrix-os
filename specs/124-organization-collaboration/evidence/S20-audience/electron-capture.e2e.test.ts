...
//   cp specs/124-organization-collaboration/evidence/S20-audience/electron-capture.e2e.test.ts tests/e2e/desktop/
...
suite(`S20 Electron Desktop evidence (${MODE})`, () => {
  ...
  let profile: string;
  let patched: { file: string; backup: string } | null = null;
  ...
  beforeAll(async () => {
    ...
    if (MODE === "org") {
      ...
      const backup = join(tmpdir(), `s20-${hits[0]}.bak`);
      ...
      writeFileSync(file, source.replace(/Reflect\.get\(\w+,\s*"organizationId"\)/, JSON.stringify(ORG_ID)));
      patched = { file, backup };
  ...
  afterAll(async () => {
    ...
    if (patched) { copyFileSync(patched.backup, patched.file); rmSync(patched.backup, { force: true }); console.log("[s20] restored renderer chunk"); }
