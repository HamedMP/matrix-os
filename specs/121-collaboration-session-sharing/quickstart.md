...
2. Choose one Cxx slice and its acceptance evidence. Read [spec](spec.md), [plan](plan.md), [data model](data-model.md) and both [HTTP](contracts/collaboration-api.md) and [realtime/execution](contracts/realtime-execution.md) contracts. A merged backend slice must remain dormant until its milestone gate passes.
