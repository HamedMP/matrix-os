...
The initial M2 wiring adds shared dispatch coordination to the existing canonical Chat orchestrator so it can preserve one queue, transcript and active-run guard. Track the required structural follow-up in [#1610](https://github.com/HamedMP/matrix-os/issues/1610): before PR4 adds terminal collaboration behavior, extract shared claim/preparation/completion/cancellation/notification coordination into a focused module, retain repository ownership of canonical state, keep personal execution unchanged, move focused tests to the extracted boundary and reduce `chat/orchestrator.ts` below 1,500 lines.
