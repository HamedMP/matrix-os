...
Provider authority is canonical Chat metadata under that same Chat row lock. Ordinary shared request payloads contain no selection, Driver, Instance, credential or resume authority. For a bound Chat, admission copies the exact current selection only when its Instance matches `bound_instance_id`, uses `bound_driver_kind`, and separately requires signed execution eligibility for that Driver and isolated adapter. The adapter capability never invents or rewrites a Provider Instance. Claim transactionally revalidates membership, epoch, execution generation/eligibility, Driver, Instance, and the complete queued selection before creating a Turn or Run. For an unbound Chat, only an owner-authored request may use a server-validated canonical catalog selection to create the first binding; the editor cannot supply, select, or alter Provider/model authority. Editors receive `owner_binding_required`/unavailable behavior and cannot establish it. The Instance is immutable; the model may change within that Instance only through existing canonical owner Chat behavior, and each admitted shared request preserves that exact canonical selection.

An unsupported binding is not remapped. In particular, a valid `claude_code` + `claude_code_default` binding executes unchanged against verified `claude-code` eligibility and is never converted to `claude_shared`. Any `claude_code` Instance executes on the `claude-code` isolated adapter; the isolated `codex` adapter executes only the pinned `codex` + `codex_default` binding, and its readiness is verified through the trusted provider catalog with generic unavailability (never Claude reconnect guidance) when the owner's Codex identity is not usable. Admission fails unavailable, and claim terminalizes any legacy or racing incompatible queued row as unavailable without changing `current_selection` or any `bound_*` column. Shared execution never loads `chat_run_adapter_state` from private Runs. Claude credential readiness is projected separately and follows the kernel credential access source the scoped run will actually use (Matrix-included access, the owner's API key, or the owner's Claude profile); Matrix-included access is platform managed and needs no probe. Owner API key and Claude profile routes are never ready on credential material alone: the trusted server-side provider catalog must validate the complete bound selection (Instance, model, options, and shared-run requirements) exactly as the first-binding path does, an `authentication_required` Instance maps to reconnect-required, and any other unavailability fails closed. Without a catalog the profile route falls back to the Claude login state and the API key route fails closed. An expired or missing owner authorization yields actionable reconnect guidance only to the owner and generic Provider-unavailable behavior to collaborators. Clients keep the last confirmed readiness when a refresh fails: an available queue is retained with a recovery notice, and an owner reconnect requirement is never promoted to available.
