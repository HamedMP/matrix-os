...
Discussion appends create no run. AI submission does not carry Provider selection, Driver, Instance, an owner principal, arbitrary root, credential or personal resume ID. Admission and claim derive and revalidate execution selection from the locked canonical Chat binding and scope capability. The adapter receives a trusted scoped execution context constructed by authority. Resume state must match Chat+scope+execution generation+adapter and derive solely from shared execution. Historical visible messages may inform fresh context; hidden private session state, personal memory and tool credentials may not. If the bound Provider Instance is unsupported, the request is unavailable without rebinding or metadata mutation; history and discussion stay usable.
