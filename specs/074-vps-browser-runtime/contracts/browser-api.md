...
- Handoff token TTL MUST NOT exceed 60 seconds. The replay store keeps consumed nonces until token expiry plus 30 seconds, caps at 10,000 entries per owner VPS process, prunes expired entries first, and fails closed instead of evicting unexpired nonces when the cap is full. Process restart inside the TTL window is an accepted residual replay risk unless a durable nonce store is configured.
