...
- **FR-028**: No user-derived prompt, command text, cwd, credential, token, sandbox path, or dynamic agent option may appear in supervisor, keeper, pane helper, provider-launch, or systemd argv, unit metadata, environment files, or journals. Cwd MUST be applied with `chdir`; prompts MUST enter providers through stdin or typed RPC; dynamic provider configuration MUST use the one-shot descriptor or an inherited FD. A provider that rejects anonymous FDs MAY receive descriptor-derived settings through an exclusive `0600` regular file on owner-only tmpfs: its name MUST be only the validated operation ID, argv MUST contain only the fixed `/proc/self/fd/3` reference, the pane MUST retain the open handle for the provider lifetime and identity-check cleanup, and the bounded TTL sweep MUST remove crash leftovers. This exception is proven for Claude 2.1.226, which rejects pipes, FIFOs, sockets, and unlinked regular files as settings sources.
