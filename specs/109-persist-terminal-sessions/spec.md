...
**Created**: 2026-07-20
**Status**: Approved for layered implementation — production activation remains gated
...
The target architecture was authorized only after both disposable-VPS spikes passed with the exact production-candidate Zellij build `v0.44.3-matrix.1` on a production-representative Ubuntu customer VPS. The immutable proof is recorded in [evidence/README.md](./evidence/README.md): exact head `f5a91fb6cd01eb30a747f04766a7ef67371acb3c`, workflow run `29958582126`, S1 pass, S2 pass, and privacy scan pass. This build is Zellij 0.44.3 source plus the reviewed Matrix resurrection patch, compiled with Rust 1.92.0 for `x86_64-unknown-linux-musl`; its source archive, patch, and resulting binary are SHA-256 pinned. Throwaway spike code and units remain evidence, not production supervisor implementation. Any later candidate change requires new complete S1/S2 evidence; it does not permit weakening an invariant.
...
- serializes the visible viewport plus the bounded rows needed below it, records a versioned numeric viewport offset, and restores that offset only in Zellij's trusted resurrection-content path;
- counts rows above, inside, and below the visible viewport inside one configured 10,000-row serialization ceiling, prioritizing the visible viewport and the immediately following rows needed to reconstruct it;
- leaves Zellij's native held-command input handling intact and never enables `--force-run-commands`.

S2 measures viewport restoration while the resurrected pane is still held behind
the native confirmation banner, before a confirmed command or newly selected
shell can emit output and legitimately move the viewport. It separately dismisses
the gate without running the prior command and verifies the complete restored
history remains bounded to 10,000 marker lines.

The checked-in `scripts/terminal-runtime/zellij/v0.44.3-matrix.1.build.json` is the single authoritative candidate record consumed by the production builder, bundle verifier, updater candidate path, remote spike, and evidence verifier. It records source archive digest `33ae61fc802b59462fed49b424893596d3aa819646bdce53d5602f714c1264fe`, patch digest `bee3d6c227402258faee58c9f57ed282a368ab39fd38e619b39d4bd5ec8f2571`, binary digest `534455dc62c8e3753918d012547d10159ee07929f570a5873a754957502a49c4`, Rust 1.92.0, the musl target, reproducibility inputs, and the exclusive build root `/tmp/matrix-zellij-build-v0.44.3-matrix.1`. The build fixes locale, timezone, source epoch, and incremental behavior, and remaps the complete fixed build root to `/usr/src/matrix-zellij`. The fixed root is also required because vendored OpenSSL embeds its build/install paths before Rust path remapping can act; a random temporary root therefore changes the final bytes. Every preview and release workflow MUST rebuild on `ubuntu-24.04`, run all four native regressions, reproduce the record, and pass the complete build directory to the host-bundle verifier. The host bundle has no released-Zellij fallback and MUST reject missing, altered, or incomplete production build inputs. Every bundle that replaces Zellij MUST preserve the prior binary and metadata as part of the same one-level application rollback generation: after services stop, the updater builds the complete root-owned snapshot under a temporary path inside the current app and moves it into place only after every copy succeeds; the existing app-to-rollback rename then commits the app and matching Zellij snapshot together. A bundle carrying candidate metadata MUST additionally validate the bundled digest, atomically rename the executable into place, and validate the installed digest. Failed or explicit application rollback MUST restore both prior Zellij files before restarting the gateway; the remote spike MUST independently reject installed bytes that differ.
...
5. The generated Zellij config does not yet explicitly enable viewport serialization. Production bundles now carry the exact S1/S2-proven `v0.44.3-matrix.1` bytes, but the existing gateway-owned runtime remains selected until the final activation layer.
...
- **FR-004**: The selected production model MUST consist of one root-owned `matrix-terminal-runtime.service`, a fixed `matrix-terminal-session@.service` template, and `matrix-terminal.slice`; the stable supervisor uses `Type=notify` and MUST report readiness only after its owner socket, keeper socket, and maintenance worker are available. It starts at boot but MUST NOT enumerate or start terminal instances automatically.
...
- **FR-016**: `live` MUST require an active ready unit, a live keeper client, an exact responsive Zellij session, and verified keeper/server/shell cgroup membership. Fresh create MUST use a command-free initial-shell layout, followed only after exact-session response and one-shot launch authorization by a fixed typed pane-helper action. Because the production Zellij candidate does not reliably return its documented pane identity, readiness MUST require successful action completion plus required workload membership in the runtime cgroup, stable across five consecutive bounded samples. Fresh workload loss MUST fail the keeper; Recover MUST never issue that action or inject confirmation input. A spawn event, receipt, attach client, or agent metadata alone MUST NOT report liveness.
...
- **FR-028**: No user-derived prompt, command text, cwd, credential, token, sandbox path, or dynamic agent option may appear in supervisor, keeper, pane helper, provider-launch, or systemd argv, unit metadata, environment files, or journals. Cwd MUST be applied with `chdir`; prompts MUST enter providers through stdin or typed RPC. Dynamic provider configuration MUST use an owner-only one-shot tmpfs record initially keyed by operation ID; before Zellij starts, the keeper MUST claim and exclusively republish it under the validated immutable runtime ID, and the pane MUST derive that ID only from its exact terminal-unit cgroup before one-shot claim. Provider payloads then use stdin, typed RPC, or an inherited FD. Matrix-managed Pi MUST use its fixed `--offline` flag to prohibit mutable startup downloads while retaining normal provider requests. A provider that rejects anonymous FDs MAY receive descriptor-derived settings through an exclusive `0600` regular file on owner-only tmpfs: its name MUST be only the validated runtime ID, argv MUST contain only the fixed `/proc/self/fd/3` reference, the pane MUST retain the open handle for the provider lifetime and identity-check cleanup, and the bounded TTL sweep MUST remove crash leftovers. This exception is proven for Claude 2.1.226, which rejects pipes, FIFOs, sockets, and unlinked regular files as settings sources.
...
- **FR-047**: The updater MUST install stable bytes and unit definitions before starting a new gateway and run `daemon-reload`. The dormant parent layer MUST be installed before the supervised activation layer so the typed updater, rather than the pre-runtime legacy updater, installs the immutable runtime generation. On the first supervised activation it MUST enable and await `Type=notify` readiness from the previously inactive supervisor before starting the fail-closed production gateway; rollback MUST stop that newly introduced supervisor before restoring its stable bytes. Later normal updates MUST leave an already-active supervisor and every terminal instance untouched. Normal update/rollback stop allowlists may contain gateway, shell, and Symphony only; they MUST exclude `matrix-terminal-session@*`, `matrix-terminal.slice`, and `matrix-terminal-runtime.service`.
...
- **FR-049**: First-release migration MUST create new runtime IDs and recoverable receipts from validated legacy names/cwd in `shell-sessions.json` and workspace records, disclose one final interruption, never claim PID adoption, and never rerun a previous command or agent. A shell record and workspace record that share a legacy display name remain distinct logical sessions: the shell keeps the original name and the workspace receives a validated collision name derived from its new immutable runtime ID. Legacy serialization is imported only if Gate S2 proves safe exact-name conversion. Metadata from already-external development sessions MAY be reconciled, but their live PIDs MUST never be adopted.
...
| `matrix-terminal-keeper` | stable helper; runtime `MainPID` | Claim descriptor, allocate PTY, launch/monitor Zellij, answer only fixed capability/color queries required by headless TUI clients, verify readiness, notify systemd |
...
1. Host boot starts the stable `Type=notify` supervisor and creates/verifies owner-only runtime directories and sockets; readiness is withheld until both sockets and the maintenance worker exist. It does not enumerate or start template instances.
2. On first activation, the updater installs the stable generation and unit, enables the supervisor, and waits for systemd readiness before starting the new gateway. Gateway startup then resolves and validates a compatible supervisor client before registering production terminal dependencies. Registration fails closed if unavailable/incompatible.
