...
The required Electron clipboard E2E exposed duplicate React runtimes: the shared UI package's locked peer resolves React 19.2.3 while Electron uses 19.2.6. Reproducing that dependency layout locally caused the same terminal-opening failure (`useState` on a null dispatcher). Electron now deduplicates React and React DOM through the renderer. The packaged terminal opens with that layout, all 25 desktop build/configuration tests pass, and the seven packaged Electron clipboard/selection cases pass when run alone, including 50 context-menu copy trials. Concurrent local Electron runs produced intermittent selection failures, so clipboard validation requires exclusive focus.
