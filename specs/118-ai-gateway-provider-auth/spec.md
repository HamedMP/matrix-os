...
- **FR-045**: Matrix-funded readiness MUST require explicit current owner/runtime policy and ledger eligibility plus a fresh authenticated, model-specific relay receipt for the exact allowed model. Process liveness, another model's success, configuration, or a legacy platform key alone MUST fail closed. Paid readiness probes MUST be disabled without an explicit fleet-wide operator count budget and must use bounded concurrency, duration, and cache freshness.
