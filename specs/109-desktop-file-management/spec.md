...
- `Keep Both` generates Finder-style numbered names and claims the final target without check-then-write. On the production Linux x64 glibc VPS, the Gateway-owned Node-API capability claims files through pinned directory descriptors with `openat(O_EXCL)` or `symlinkat`. Directory copies are built under one high-entropy operation-owned sibling staging name, with all nested writes anchored to its opened descriptor, and are published to the public target only through `renameat2(RENAME_NOREPLACE)`. Only a top-level target conflict without a retained staging artifact may advance to another candidate name.
- Once execution begins, each item completes independently. The service does not attempt a batch-wide rollback.
- A same-filesystem item move is one atomic `renameat2(RENAME_NOREPLACE)` between source and target parent descriptors opened beneath the owner home with `openat2`. That syscall is the linearization point: it moves the entry at the authorized source pathname at commit time and never overwrites a target claimant. Cross-device moves fail closed with a stable generic result; they do not fall back to copy-then-delete. Unsupported OS, architecture, libc, kernel, or filesystem combinations also fail closed.
- A nested directory-copy or publish failure retains and reports exactly the one normalized, operation-owned staging path. It is another acceptable reconciliation state: the Gateway never recursively deletes a staged path that concurrent owner activity may have changed, never installs partial contents at the public target, and duplicate/Keep Both logic does not fan out additional candidate names once a retained artifact exists.
...
Both routes return the normalized resulting relative path and its fresh capability object. They use the shared Zod name/path schemas and mutation policy, re-authorize the complete source and target paths against the authenticated owner immediately before the filesystem write, exclusively create a new file/directory, and reject an occupied rename target rather than overwriting it. Directory self/descendant targets are rejected before any target-parent creation. The production Gateway then delegates structural namespace mutation to its dependency-free asynchronous Node-API capability for Linux x64 glibc. Parent traversal is pinned beneath the owner home with `openat2(RESOLVE_BENEATH | RESOLVE_NO_MAGICLINKS | RESOLVE_NO_SYMLINKS | RESOLVE_NO_XDEV)`; file creates remain descriptor-relative and no-replace, directory copies stage through an opened sibling descriptor before `renameat2(RENAME_NOREPLACE)` publication, and rename uses direct `renameat2(RENAME_NOREPLACE)`. Recursive copy is bounded to 10,000 entries and depth 128, compares device, inode, full mode, size, mtime, and ctime across identity/readable opens and after reads, preserves supported permission bits, copies symlinks without dereferencing them, and returns one normalized `partialPath` after a post-stage failure. Exact denied sources and ancestors containing denied content are never copyable or duplicable, while legacy copying from protected `system` and `agents` sources remains compatible. The addon is loaded only by Gateway and shipped precompiled in the host bundle; a customer VPS never needs a compiler. MAT-268 hardens and replaces the Desktop use of the legacy `mkdir`, `touch`, and `{ from, to }` rename payloads; compatibility handling for other callers remains explicitly tested at the route boundary.
...
- Bound native recursive copy to 10,000 entries and depth 128. Open source entries and staging directories relative to pinned descriptors; reject mount crossings with `RESOLVE_NO_XDEV`; compare full stability metadata before applying modes or reading data; source symlinks are copied as links and never dereferenced.
- Support structural mutations initially on Linux x64 glibc only. Missing addon bytes, unsupported platforms/libcs/kernels, `openat2`/`renameat2` unavailability, and `EXDEV` moves fail closed without a pathname-based JavaScript fallback.
- Same-UID residual boundary: Linux has no generally available atomic "create directory and return its descriptor" primitive and cannot publish a directory by descriptor with a no-replace identity condition. A malicious process with the owner's UID can discover operation staging names (including through `/proc`), rename/replace them between the addon's identity check and pathname-based `renameat2`, or access the open directory through process descriptors. Matrix therefore cannot claim staged-byte integrity against an actively adversarial same-UID process. It does guarantee that nested copy writes never target a competitor installed at the public target, that `RENAME_NOREPLACE` never overwrites or merges with that claimant, and that a changed/failed staging path is retained and reported rather than recursively deleted.
...
2. **Linux native seam:** Ubuntu x64 glibc CI compiles and loads the real Node-API addon, then exercises target claimants, parent-symlink attacks, recursive bounds, metadata preservation, and the public Gateway operation seams.
3. **Directory subscription seam:** public `FileDirectorySubscriptionHub` methods plus authenticated WebSocket integration exercise caps, TTL eviction, shutdown drain, frame validation, revisions, and failed-sender isolation.
4. **Desktop component seam:** React Testing Library drives rendered Files rows, menus, inline editors, folder picker, pointer/modifier/keyboard selection, drag events, safe errors, and public `ApiClient` methods without asserting private React state.
5. **Live Electron seam:** the packaged-equivalent macOS Desktop runs against a real Gateway; create, rename, multi-select, drag/move, Trash, partial failure, and external Terminal/Agent changes are observed in the UI.
