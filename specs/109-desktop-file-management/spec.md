...
- `Keep Both` generates Finder-style numbered names and claims the final target without check-then-write. On the production Linux x64 glibc VPS, the Gateway-owned Node-API capability claims each target through a pinned directory descriptor with `mkdirat`, `openat(O_EXCL)`, `symlinkat`, or `renameat2(RENAME_NOREPLACE)`. Only a top-level target conflict may advance to another candidate name.
...
- Bound native recursive copy to 10,000 entries and depth 128. Open source entries and target directories relative to pinned descriptors; source symlinks are copied as links and never dereferenced.
