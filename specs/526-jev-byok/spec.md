...
- **FR-027**: When Hermes runs a saved bot on a shared computer, its local Matrix MCP requests MUST carry the authenticated Chat run owner with a gateway-verifiable proof. The MCP MUST NOT silently fall back to the VPS owner's integrations for a collaborator's bot.
  Hermes sanitizes the stdio MCP subprocess environment, so the host-installed launcher recovers only a complete signed Run identity from a bounded process-ancestor chain before its own credential-isolating environment reset. A Hermes Chat MCP launch without that delegation fails closed instead of inheriting the VPS owner's account. The gateway MUST re-sign its authenticated request principal with the per-machine Platform token before proxying an integration request. Platform MUST verify that proof, enforce the machine's owner or Preview collaborator access list, and resolve integrations for that actor; a missing proof on a legacy owner request may retain the owner fallback, while a malformed proof MUST fail closed. The saved bot also checks the inventory email against the Gmail connection selected by the current user before reading mail.
