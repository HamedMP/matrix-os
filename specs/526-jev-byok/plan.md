...
### Provenance follow-up: authorization and settlement contract
...
- **Security and idempotency**: only the authenticated internal relay may set these fields; callers cannot select a model, payer, endpoint, or price. Compare amount and provenance on settled retries, reject altered tuples without a second debit, and retain the owner-scoped hold for a new versioned reservation whose exact finalization omits or mismatches provenance. The existing owner admission lock serializes new holds; finalization takes the reservation row lock and writes provenance, settlement response, balance changes, and ledger debits within one transaction. No database transaction spans the external call.
- **Unknown outcomes**: a started call with no trustworthy usage stays in flight with its hold and no debit or fabricated model/version. Evidence-bound manual reconciliation may settle such a versioned hold with null provenance if the upstream receipt is unavailable; it records the reviewer, evidence reference, and verified amount. This explicit orphan state does not imply that the provider did not bill. A pre-dispatch authorization/version mismatch releases the hold and never calls Jev.
...
- **Repository extraction plan**: `ai-funded-metering-repository.ts` already exceeds 1,000 lines. In a focused follow-up, move `settleReservationInternal` and its reservation-locked provenance/idempotency checks into `ai-funded-reservation-settlement.ts`, passing the transaction executor and existing metering helpers explicitly; retain the public repository method as a thin wrapper. Then move owner-locked authorization into a separate module. Preserve the lock order, one-transaction debit and response write, and legacy/manual paths with the current funded metering fixtures and an independent-connection PostgreSQL check when an isolated server is available. This provenance slice adds only the checks at the existing settlement boundary to avoid mixing the behavior change with that refactor.
...
Retention must preserve that promise across restarts and across the seven-day result-data window. The Gateway Postgres primary key on `(owner_id, idempotency_key)` and payload hash remain the replay authority. Completed result JSON becomes eligible for pruning after seven days; the retained `completed_pruned` tombstone keeps the replay barrier. A matching retry gets an explicit `result_expired` response without acquiring a credential or dispatching, even if its old JSON has not yet reached a maintenance batch; a different payload still conflicts. Unknown outcomes retain their tombstone. Claim locks and resolves only its requested key, including targeted expiry, in one database transaction without a relay call inside it. It does no global cleanup on the request path. Gateway initializes the owner-Postgres retention lifecycle independently of paid Jev eligibility, so disabling the funded route does not strand old results. A shared owner database without its resolved connection URL fails Jev initialization rather than silently borrowing that shared pool for maintenance. An independent maintenance tick runs once at startup and every minute on a Jev-owned single-connection Postgres pool, processing at most 100 eligible rows per tick with no overlap per instance. Its connection acquisition and query have bounded timeouts, and idle pool errors are logged by name only. Shutdown cancels future ticks, waits up to five seconds for an active sweep, then force-releases only its own active client and closes its owned pool before the shared owner database is destroyed. Jev never closes the shared pool. An idle result is therefore pruned without another evaluation, but a backlog can delay physical JSON removal beyond seven days; monitor that backlog. This avoids a global row update per request and cross-key lock inversion across Gateway instances. An additive, idempotent migration expands the existing table status constraint without dropping rows. Historical keys already deleted by the old cleanup cannot be reconstructed. Before rollback to a binary with the old delete behavior, disable the Jev route and drain traffic; that binary would remove retained unknown rows and does not understand `completed_pruned`.
