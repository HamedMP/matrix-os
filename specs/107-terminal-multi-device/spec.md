...
- **FR-022**: The gateway MUST provide an idempotent migration operation (`POST /api/terminal/workspaces/migrate`) that converts legacy sessions into workspace tabs: for each exited or idle legacy session (no attached clients), create a tab in the target workspace with the same display name and, when recorded, the same working directory; MOVE (rename) the legacy session's scrollback file to the new tab's scrollback key BEFORE deleting anything; then delete the legacy zellij session and prune its registry entry through a migration-specific path that MUST NOT remove the moved scrollback file (the normal session-delete path removes scrollback with the session, so migration cannot reuse it as-is). If the rename fails, the migration for that session aborts and reports it as skipped rather than deleting history. The registry MUST begin persisting `cwd` at session/tab creation time from the first workspace-aware release; legacy sessions predating cwd recording migrate with the Matrix home as cwd (an exited zellij session cannot be queried for its original cwd — documented limitation). Live legacy sessions (attached clients or running foreground processes) are reported as skipped, never force-killed — a running process cannot be moved between zellij sessions.
...
- **FR-017**: WS output delivery MUST implement bounded per-subscriber flow control. When a socket's buffered amount exceeds the high-water mark (default 1 MiB), the gateway MUST synchronously detach that subscriber and close it with retryable WebSocket code `1013`; it MUST NOT skip one frame and later resume live delivery on the same connection. The client preserves its next expected sequence and reconnects with `fromSeq` set to that first missing sequence. While replay is loading, that subscriber receives no live frames and owns no output queue; the gateway catches it up from the bounded shared replay/scrollback stores before enabling live delivery. Replay/live catch-up work is capped at eight passes, after which the subscriber is closed with `1013` to retry. If the first missing sequence is no longer available, the gateway sends the generic `replay_unavailable` error and closes only that subscriber. Backpressure or send failure on one subscriber MUST NOT pause the shared recorder, stall persistence, or interrupt healthy subscribers.
- **FR-018**: Exited sessions MUST be reaped by a periodic sweep (default: exited > 7 days), deleting the zellij session, pruning the registry entry, and removing scrollback files. The sweep MUST be symlink-safe and its timer cleared on shutdown. Registry entries predating workspace support (no `kind` metadata) are EXEMPT from reaping until the FR-022 migration operation has shipped and the automatic migration pass (FR-023) has run on that home — Phase 1 reaps only sessions created with `kind` metadata, so pre-existing exited sessions cannot be deleted before users get the chance to convert them.
...
- **Auth matrix** (no new principals; the customer VPS gateway is single-tenant, owner-only):

| Surface | Principal | Mechanism | Allowed actions |
|---|---|---|---|
| `/api/terminal/sessions*` (list/create/delete), workspace CRUD, `/sessions/:name/tabs*` (list/create/rename/close/focus) | VPS owner | Existing gateway bearer-token auth on the `/api/terminal` mount (unchanged) | Full CRUD on own sessions/workspaces/tabs |
| `POST /api/terminal/workspaces/migrate` | VPS owner | Same bearer-token auth; mutating: `bodyLimit` + Zod body schema; covered by the shared session-creation rate limiter | Run idempotent legacy migration |
| `POST /api/terminal/run` (changed by FR-024 to create workspace tabs) | VPS owner | Same bearer-token auth; mutating: `bodyLimit` + Zod body schema (command, workspace selector); covered by the shared session-creation rate limiter since it now creates terminal resources | Run a command in a new workspace tab |
| `/ws/terminal`, `/ws/terminal/session` | VPS owner | Existing WS token auth including the browser query-token allowlist registration for these exact paths (unchanged) | attach, input, resize/viewport-hint, detach, destroy |
| Unauthenticated / any other principal | — | — | No access; generic 401, no route-shape disclosure |
| Platform-internal | — | Not used by these routes | — |
...
- **Recorder PTY dies**: re-election on next output or attach event; any unavailable seq range is surfaced explicitly and later output is not applied across it.
...
- **Slow/dead client**: flow control detaches and closes only that subscriber with retryable code `1013`; the normal close path removes it immediately, stale-attach eviction remains a fallback for partitioned connections, and other clients remain unaffected.
...
1. **Phase 1 — output pipeline**: send-first delivery, recorder election, coalesced persistence, WS flow control, client sequence validation/reconnect, and exited-session reaper (metadata-tagged sessions only; pre-existing legacy sessions are exempt per FR-018 until Phase 3 migration has run). This phase is independently shippable across the gateway and terminal clients.
