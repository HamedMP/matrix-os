...
- **FR-017**: WS output delivery MUST implement flow control: when a socket's buffered amount exceeds the high-water mark (default 1 MiB), that client's attach PTY is paused; resumed below the low-water mark (default 256 KiB). If the affected client is the current recorder, the gateway MUST re-elect the recorder (FR-005) before pausing that PTY; if no other live attach is eligible, the recorder PTY is NOT paused — instead delivery to the slow socket is suspended (frames skipped for that socket, which is marked stale for FR-019 eviction, and the skip reported to it as a replay-evicted range on recovery) while the recorder stream continues feeding persistence. Socket backpressure must never stall persistence of output that other clients are receiving live.
...
- **Startup**: the stable root supervisor uses `Type=notify`, reporting ready only after both protocol sockets and its maintenance worker exist. It starts at boot, but no terminal template instance is enabled or automatically started. On the first supervised activation, the updater waits for that readiness before starting the production gateway. The gateway fails closed unless it can complete the frozen protocol-v1 compatibility probe. Gateway-owned attach PTYs and timers are drained on shutdown without stopping supervised runtime cgroups.
