...
| Mobile project-first task/thread navigation with Conversation and Kanban modes | Phase 22.1 adds gateway-validated project hydration, project/task/thread routes, multi-thread task groups, safe selected-reference persistence, and foreground/reconnect reconciliation in `apps/mobile/components/agents/agent-project-workspace-screen.tsx` plus the `MB-001` through `MB-004` evidence named in `acceptance-tests.md`; same-thread Conversation, Kanban, and device evidence remain outstanding | **Partial** |
