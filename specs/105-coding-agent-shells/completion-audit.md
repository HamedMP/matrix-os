...
| Mobile SDK 57 coding-agent workspace and phone-first routes | `apps/mobile/app/(tabs)/workspaces.tsx`; `apps/mobile/app/agents/index.tsx`; `apps/mobile/app/agents/projects/[projectId]/index.tsx`; `apps/mobile/app/agents/projects/[projectId]/board.tsx`; `apps/mobile/app/agents/[threadId].tsx`; `apps/mobile/components/agents/agent-project-workspace-screen.tsx`; focused tests listed under Phase 22 in `acceptance-tests.md` | Implemented; physical-device gate pending |
| Legacy Web Workspace projection remains read-only for coding-agent state pending retirement | `shell/src/components/workspace/WorkspaceApp.tsx`; `tests/shell/workspace-app.test.tsx`; browser section in `current-state.md` | Superseded by `specs/119-os-view-parity/spec.md` |
...
The Phase 22 mobile implementation is split into ready-for-review stacked PRs with implementation and review evidence:

- PR #911, `57aff474d`, implements project-first routes, bounded safe-reference persistence, hydration, and stale-reference reconciliation. Review follow-up resolves a valid routed project through its authoritative workspace even when it is beyond the first summary page, exposes all three bounded pagination cursors, gates every create affordance on `codingAgentsThreadCreate`, and redirects project deep links through the guarded Agents entry when the rollout flag is disabled. The full mobile run passed 36 suites / 384 tests; mobile lint and TypeScript passed. All review threads are resolved.
- PR #912, `6d02271cb`, implements the capability-gated same-thread turn composer, retry/idempotency/offline recovery, and project-aware new-chat creation. Review follow-up parses the route's actual `{ error: CreateAgentTurnError }` envelope, resolves composer project context beyond the first summary page, scopes generated request counters to each composer instance, offers same-thread turns only in gateway-accepted lifecycle states, and hides the composer from legacy unassigned threads that the gateway cannot accept. The full mobile run passed 36 suites / 403 tests before the final focused unassigned-thread regression passed 61 tests; mobile lint and TypeScript passed. Greptile reviewed the exact head at 5/5 and all review threads are resolved.
- PR #913, `fd7db1332`, implements Conversation/Kanban routing, canonical task columns, bounded aggregates, multi-thread routing, tablet-width rendering, and cross-view identity continuity. Review follow-up explicitly seeds the normal project route as Conversation so persisted Kanban state cannot reopen on that URL, gates direct board links behind the same mobile-workspace rollout flag, and contains the corrected project-route and turn-lifecycle guards. The two final affected suites passed 63 tests; mobile lint and TypeScript passed. Greptile reviewed the exact head at 5/5 and all review threads are resolved.
- PR #914 at implementation head `3f37e0dc3` adds the authenticated computer projection and native chooser, canonical hosted journey/app-session routing, and a dedicated mobile Chats tab that restores a live project and preserves project/task/thread identity. It also removes the native portrait lock so the required phone/tablet landscape layouts can be exercised. Late review hardening validates each projected machine independently, quarantining a malformed legacy row instead of failing the owner's entire list. The full mobile run passed 43 suites / 436 tests; mobile TypeScript, mobile lint, root typecheck, and the pattern scan passed. Title, gate, Neo worker, preview build, and preview deployment checks passed, and all review threads across PRs #911-#914 are resolved. Greptile's first exact-head pass scored 4/5 only because it could not see the unchanged preview-service invariant in the PR diff; route, service, persisted-row test evidence was supplied and an exact-head re-review was requested.
...
- Exact implementation-head workflow run `29147546571` built, published, and deployed `v2026.07.11-pr914-3f37e0d-r29147546571` to `pr-914`.
...
- The Expo SDK 57 dev client is installed on the paired physical iPhone 14 Pro, Metro bundled and relaunched the exact implementation head successfully, and the process remained live without a runtime crash. User confirmation of the rendered list, runtime switch, dedicated Chats tab, and subsequent project/task/thread walkthrough is still pending and is not inferred from deployment success.
