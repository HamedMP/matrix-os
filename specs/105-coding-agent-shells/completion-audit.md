...
- PR #912, `9991cc5c6`, implements the capability-gated same-thread turn composer, retry/idempotency/offline recovery, and project-aware new-chat creation. Review follow-up parses the route's actual `{ error: CreateAgentTurnError }` envelope, resolves composer project context beyond the first summary page, scopes generated request counters to each composer instance, and offers same-thread turns only in gateway-accepted lifecycle states. The full mobile run passed 36 suites / 403 tests; mobile lint and TypeScript passed. All review threads are resolved.
- PR #913, `7f384c1d8`, implements Conversation/Kanban routing, canonical task columns, bounded aggregates, multi-thread routing, tablet-width rendering, and cross-view identity continuity. Review follow-up explicitly seeds the normal project route as Conversation so persisted Kanban state cannot reopen on that URL, and the restacked head contains the corrected project-route and turn-lifecycle guards. The two affected focused suites passed 62 tests; mobile lint and TypeScript passed. All review threads are resolved.
- PR #914 through restacked implementation head `516ecf3bf` adds the authenticated computer projection and native chooser needed to test the stack on the disposable preview computer and contains the corrected lower stack as ancestors. It also removes the native portrait lock so the required phone/tablet landscape layouts can be exercised. The canonical-origin mobile run passed 42 suites / 427 tests; mobile lint and TypeScript passed. All review threads are resolved. GitHub's current check set and Greptile review remain the authoritative merge gate for documentation-only commits above this implementation head.
