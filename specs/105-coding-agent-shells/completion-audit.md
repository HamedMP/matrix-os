...
- PR #914 through implementation commit `f6322b3ab` adds the authenticated computer projection and native chooser needed to test the stack on the disposable preview computer and contains the exact current lower stack as ancestors. It also removes the native portrait lock so the required phone/tablet landscape layouts can be exercised. The canonical-origin mobile run passed 42 suites / 422 tests; mobile lint and TypeScript passed. The prior exact docs head passed all checks and Greptile 5/5; the new orientation head requires its own exact-head review.
