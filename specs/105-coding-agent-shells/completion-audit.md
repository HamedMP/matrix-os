...
| One canonical task supports several independently selectable coding threads | The gateway project workspace projects several threads per task and separates project chats, but relation-enforcing creates and desktop/mobile navigation remain incomplete; legacy task UI still has a singular session link | **Incomplete** |
