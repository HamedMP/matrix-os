...
| One visible chat is one resumable thread and follow-up messages create turns in that same provider conversation | The bounded turn route/store now persists idempotent accepted turns and one-active-turn ownership, but production registration, provider resume dispatch, active-turn release, and shell clients remain incomplete | **Incomplete** |
| One canonical task supports several independently selectable coding threads | The gateway project workspace projects several threads per task, separates project chats, and validates new shell-created project/task relations, but explicit legacy reassignment and desktop/mobile navigation remain incomplete; legacy task UI still has a singular session link | **Incomplete** |
