...
- Codex structured thread events: the gateway has a strict external JSONL normalizer for assistant, tool, file, provider identity, and turn-outcome records plus owner-scoped, durable-before-publish ingestion that deduplicates replayed event ids and rejects cross-thread or reserved lifecycle events. The Matrix-owned runner and watcher now feed live Codex records through replay and WebSocket publication while keeping raw commands, output, stderr, paths, and provider failures out of client events. The installed CLI must remain in the verified `0.144.0` through `0.144.3` range. A scheduled check resolves the latest published package and verifies both the tagged exec schema and generated experimental app-server schema against the same version range. Newer, older, invalid, or drifted protocols fail closed. The app-server approval/input methods are pinned but not advertised as available until the control runtime and end-to-end decision tests land. Gateway-restart watcher recovery remains coupled to [#936](https://github.com/HamedMP/matrix-os/issues/936), which tracks preserving live workspace sessions across service restart.
- Codex app-server requests: the gateway now has a strict bounded normalizer for the pinned command, file-change, permission, and structured-input request methods. It emits only generic Matrix approval descriptions or validated bounded questions and returns native JSON-RPC/question correlation data as server-private control metadata. Raw commands, provider thread/turn/item ids, paths, hosts, permission payloads, and native request ids do not enter thread events. The long-lived control process, durable pending-request lifecycle, decision response transport, capability enablement, and real-process smoke remain deferred to the next runtime slice.
