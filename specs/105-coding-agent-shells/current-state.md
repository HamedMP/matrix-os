...
- Codex structured thread events: the gateway now has a strict external JSONL normalizer for assistant, tool, file, provider identity, and turn-outcome records plus an owner-scoped, durable-before-publish ingestion method that deduplicates replayed event ids and rejects cross-thread or reserved lifecycle events. The contract pins reviewed versions through `0.146.0` with exact tagged JSONL and generated app-server schema digests, plus a scheduled published-version drift check. Codex `0.146.0` keeps the reviewed exec event source byte-identical to `0.145.0`; its generated app-server changes add external-agent import history and optional configuration fields while preserving the Matrix-consumed approval request methods. Runtime transcript capture and live watcher wiring remain in the next stacked backend slice; until then, existing workspace sessions still emit only their startup projection.
