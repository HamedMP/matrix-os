...
**Branch stack**: `spec/coding-agent-shells` plus stacked implementation branches through `105-coding-agent-mobile-preview-route`
**Updated**: 2026-07-07
...
- File read: `FileReadRequestSchema` and `FileReadResponseSchema` for bounded owner-worktree text snapshots.
...
- When the runtime advertises `codingAgentsFiles`, review details can load one selected bounded file text snapshot through the authenticated gateway client.
- Safe generic review error state if review summaries are unavailable; review failures do not drop the runtime summary dashboard.
...
- Preview rows navigate to `/agents/preview` with bounded preview summary params. The route validates `PreviewSessionSummarySchema`, renders HTTPS origins through the existing `AppRuntimeFrame`, and rejects non-HTTPS origins without rendering raw URLs or persisting preview state.
