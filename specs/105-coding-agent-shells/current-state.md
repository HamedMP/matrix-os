...
- Current follow-up behavior seeds a new thread with a structured reference. It does not append a user turn and resume the same provider conversation in the selected thread.
...
| `/api/coding-agents/threads/:threadId/turns` | `POST` | Implemented, not registered | Auth/body/param/request/owner/relation/state checks, atomic persisted idempotency, one-active-turn ownership, bounded normalized dispatch, server-only resume state, terminal request-body cleanup, timeout/abort handling, and startup/shutdown release are implemented and fake-provider tested. Gateway startup registration and capability enablement wait for the workspace provider resume adapter. |
