...
- File read/write: `FileReadRequestSchema` and `FileReadResponseSchema` for bounded owner-worktree text snapshots; `FileWriteRequestSchema` and `FileWriteResponseSchema` for bounded owner-worktree UTF-8 writes with a required base etag or explicit create intent.
