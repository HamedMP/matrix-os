...
- Codex structured thread events: the gateway has a strict external JSONL normalizer for assistant, tool, file, provider identity, and turn-outcome records plus owner-scoped, durable-before-publish ingestion that deduplicates replayed event ids and rejects cross-thread or reserved lifecycle events. The Matrix-owned runner and watcher now feed live Codex records through replay and WebSocket publication while keeping raw commands, output, stderr, paths, and provider failures out of client events. The installed CLI must remain in the verified `0.144.0` through `0.144.3` range, and a scheduled check detects newly published versions or tagged schema drift. Gateway-restart watcher recovery remains coupled to [#936](https://github.com/HamedMP/matrix-os/issues/936), which tracks preserving live workspace sessions across service restart.
