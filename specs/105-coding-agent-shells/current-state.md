...
| `/api/coding-agents/threads/:threadId/turns` | `POST` | Implemented, not registered | Auth/body/param/request/owner/relation/state checks plus atomic persisted idempotency and one-active-turn ownership are implemented and tested. Gateway startup registration and capability enablement wait for the bounded provider dispatcher so no accepted turn can be stranded. |
