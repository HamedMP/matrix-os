...
- When the runtime has no projects, the composer offers scratch creation or GitHub import through canonical `POST /api/projects`. It accepts only the validated returned slug, refreshes `RuntimeSummarySchema`, and enables thread creation only after that slug appears as an available canonical project. Project mutation details and repository URLs stay transient.
