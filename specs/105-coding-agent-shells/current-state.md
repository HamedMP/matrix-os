...
- Current desktop/mobile follow-up controls still seed a new thread with a structured reference. The gateway now supports same-thread turns and workspace-session resume, but shell clients have not switched to that route yet. Delivering input to a running workspace session settles that turn without marking the thread complete. Codex provider turn records update the canonical thread lifecycle, while canonical session-stop reconciliation remains the fallback for terminal closure and runtime failure.
...
- Codex structured thread events: the gateway has a strict external JSONL normalizer for assistant, tool, file, provider identity, and turn-outcome records plus owner-scoped, durable-before-publish ingestion that deduplicates replayed event ids and rejects cross-thread or reserved lifecycle events. Coding-thread launches now use the Matrix-owned app-server control runner, while the exec JSONL runner remains a directly tested compatibility path. Both feed the same watcher, durable replay, and WebSocket publication boundary without exposing raw commands, output, stderr, paths, or provider failures. The installed CLI must remain in the verified `0.144.0` through `0.144.3` range. A scheduled check resolves the latest published package and verifies both the tagged exec schema and generated experimental app-server schema against the same version range. Newer, older, invalid, or drifted protocols fail closed. Approval/input capability remains disabled until the gateway submission client is wired and end-to-end decisions pass. Gateway-restart watcher recovery remains coupled to [#936](https://github.com/HamedMP/matrix-os/issues/936), which tracks preserving live workspace sessions across service restart.
- Codex app-server requests: the gateway has a strict bounded normalizer for the pinned command, file-change, permission, and structured-input request methods. The long-lived runner owns a capped local control socket, bounded pending-request and replay registries, request expiry, terminal cleanup, and decision response transport. Unsupported decisions are filtered, malformed or duplicate requests receive a fail-closed response, and only generic Matrix approval descriptions or validated bounded questions enter events. Raw commands, provider thread/turn/item ids, paths, hosts, permission payloads, native request ids, and answer text remain server-private. Gateway submission-client wiring, capability enablement, and real-process preview smoke remain deferred to the next slices.
