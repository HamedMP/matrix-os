...
- [ ] T018 [P] Tests for header-injection matcher in `tests/desktop/header-injection.test.ts`: injects only for exact gateway origin (http+ws upgrades), never for embed partitions/foreign origins/subdomain confusion (`evil-app.matrix-os.com.attacker.tld`)
