...
2. Selecting a profile reads URL and cookie-domain metadata and shows counts per website. The user selects sites. Secret values are not previewed.
   The preview keeps at most 5,000 recent website entries. A user can enter a domain manually when it is absent from the bounded list; the same host schema and source selection apply at import.
...
4. The 1Password picker invokes the local `op` CLI using desktop app integration. It lists local accounts first, asks the owner to select an account when more than one is present, then passes that account ID to both Login metadata listing and selected item retrieval. Only website, username, and password are imported; OTP fields, passkeys, other categories, and attachments are excluded.
...
- Password vault operations verify account ownership both before and after asynchronous I/O. The list handler checks again before returning usernames, and plaintext export checks immediately before publishing the destination file.
...
- Vitest covers Chromium encryption and cookie-domain integrity, malicious URLs and partitioned cookies, selected-site isolation using synthetic SQLite fixtures, the encrypted vault, 1Password account and item selection, IPC schemas, and current-origin fill gating.
