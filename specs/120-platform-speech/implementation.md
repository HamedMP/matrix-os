...
- `DELETE /api/speech/transcriptions/:requestId`: idempotent while cancellation remains possible, with `bodyLimit` even for an empty body. Return whether execution had already started; return conflict after terminal completion because the server cannot retract a committed response. Do not promise a refund or immediate provider cancellation.
...
| Dispatching + cancel requested | Serialize cancellation and completion on the operation row. If cancellation wins, abort transport best-effort and suppress server delivery; reconcile billable usage independently. If terminal completion wins, a later DELETE conflicts. The client generation fence independently suppresses late draft insertion. |
