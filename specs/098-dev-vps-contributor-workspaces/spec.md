...
`<machine-id>` means the stable, non-secret Matrix VPS machine identifier assigned during customer VPS provisioning. It must not be the user's handle, email, raw public IP, or an unvalidated hostname. If the existing provisioning identifier is not DNS-safe, the platform should mint an opaque DNS-safe dev routing slug and store the mapping platform-side.
