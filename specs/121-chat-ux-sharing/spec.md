...
- File references and attachments reveal the matching Chat inspector, preserving Chat identity. Attachments resolve in owner home; Project references use the canonical execution root. Existing safe preview APIs remain authoritative. OM-63's changes/diff workflow remains separate.
- HTTP(S) links route to Matrix Browser. Other schemes never become Browser navigation requests.
- User messages support mixed text, image previews, and file cards; long text expands without losing attachments. Image loading, failure, and retry are explicit.
...
| GET /share/chats/:token | Public bearer capability | 256-bit random token, SHA-256 lookup; expiry; bounded rate limit |
