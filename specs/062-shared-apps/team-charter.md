...
- **T095 Coverage gaps** — All six 062 gateway files are below the spec-required thresholds. This unblocks a post-merge regression window. **Owners per file** (DM sent 2026-04-12):
  - ~~`group-sync.ts`: 80.5% stmts / 60.2% branches~~ **83.07% stmts / 63.49% branches / 91.2% funcs / 84.17% lines** (commit `45d4db3`, crdt-engine, +13 tests, 90 total, verified 2026-04-12). Improvement from 80.5%/60.2%. Remaining gaps are structural dead code (ULID Math.random fallback, Node 24) and integration-only paths (defaultFetchAppBundle). **DEFERRED** per lead-integrator — not blocking T101. Follow-up PR item.
  ...
  - ~~`group-registry.ts`: 89.6% stmts / 66.7% branches~~ **DEFERRED** (lead-integrator decision 2026-04-12, not blocking T101). Remaining gaps are defensive paths that fire rarely in production (slug collision suffix, corrupt manifest quarantine). Follow-up PR item.
  - ~~`group-routes.ts`: 90.5% stmts / 79.8% branches~~ **DEFERRED** (lead-integrator decision 2026-04-12, not blocking T101). Remaining gaps are Matrix error type discrimination paths. Follow-up PR item.
  - ~~`group-ws.ts`: 80.3% stmts / 66.7% branches~~ **99.24% stmts / 92.06% branches / 100% funcs / 100% lines** (commit `541677a`, collab-shell, 39 tests, verified 2026-04-12). Dead `fanOutUpdate` function removed (11 lines). Remaining 8% branch gap is structurally capped: `if (oldest)` (line 139, logically unreachable when `set.size >= cap`), `if (!set) return` (line 151, same), `if (update.length > 0)` (line 262, empty-update skip). All require mocking internal Set behavior. Accepted as-is — above 95% stmt/line/func targets. Branch gap formally deferred per lead-integrator.
...
**Status 2026-04-12**: All 6 HIGH spec gaps closed (commit `3dcd1b6`). All 2 HIGH code gaps closed (group-platform commits). All MEDs closed (T094 at `e3f4726`, F1 impl at `a3efcb2`, S2/C1/S4/W1 in spec at `3dcd1b6`). I1 deferred. T095 coverage: group-tools 100%, matrix-sync-hub 97.4%/87.9% (structural cap, accepted). Four remaining files (group-sync, group-registry, group-routes, group-ws) deferred per lead-integrator — not blocking T101. All four noted in PR description under "Known coverage gaps — follow-up PR".
