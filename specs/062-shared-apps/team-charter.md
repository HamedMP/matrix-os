...
- **T095 Coverage gaps** — All six 062 gateway files are below the spec-required thresholds. This unblocks a post-merge regression window. **Owners per file** (DM sent 2026-04-12):
  - ~~`group-sync.ts`: 80.5% stmts / 60.2% branches~~ **DEFERRED** (lead-integrator decision 2026-04-12, not blocking T101). Remaining gaps are deferred-spec-amendment paths (snapshot-policy daemon, ACL-state handler) that require non-trivial fixture, structural dead code (ULID Math.random fallback, Node 24), and integration-only paths (defaultFetchAppBundle). Follow-up PR item. C1/F1 fixes verified correct at `a3efcb2`.
  ...
  - ~~`group-ws.ts`: 80.3% stmts / 66.7% branches~~ **DEFERRED** (lead-integrator decision 2026-04-12, not blocking T101). Remaining gaps require non-trivial binary Yjs protocol fixture (onMessage syncStep1/syncStep2/update dispatch, handleUpgrade ACL branch). Follow-up PR item.
