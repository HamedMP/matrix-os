...
- [~] PR 30 review remediation in progress — tracker: `specs/066-file-sync/pr-review.md` (trusted platform-proxy refactor now removes tenant-visible R2 credentials, `PLATFORM_DATABASE_URL`, and container JWT-signing-secret needs; latest blocker/worth-fixing wave also aligns sync identity on `MATRIX_USER_ID`, hardens home-mirror local push semantics, makes share accept/revoke transactional, and fixes watcher async-error handling; host suites cover gateway auth-jwt, platform sync-jwt, presign, r2-client, app-db, home-mirror, sync-client oauth/config/ipc, routes, user-id wiring, platform-r2-client, and the latest blocker regression wave `249/249`, while Docker verifies platform device-flow + device-routes + proxy-routing + home-mirror-env + orchestrator + internal-sync-routes + middleware short-circuit `136/136`)
...
- [x] Gateway JWT public-key cache now drops rejected RS256 import promises so
  a transient bad PEM does not require a process restart to recover.
...
- [x] Host: `pnpm test tests/gateway/auth-jwt.test.ts tests/gateway/auth-jwt-cache.test.ts` (`13/13`).
