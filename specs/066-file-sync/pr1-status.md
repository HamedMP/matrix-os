...
- [~] PR 30 review remediation in progress — tracker: `specs/066-file-sync/pr-review.md` (trusted platform-proxy refactor now removes tenant-visible R2 credentials, `PLATFORM_DATABASE_URL`, and container JWT-signing-secret needs; host suites cover gateway auth-jwt, platform sync-jwt, presign, r2-client, app-db, home-mirror, sync-client oauth/config/ipc, routes, user-id wiring, and platform-r2-client `146/146`, while Docker verifies platform device-flow + proxy-routing + home-mirror-env + orchestrator + internal-sync-routes + middleware short-circuit `103/103`)
