...
- [~] PR 30 review remediation in progress — tracker: `specs/066-file-sync/pr-review.md` (latest daemon conflict-recovery, sync-route JSON/metrics/share validation, sharing query-shape, and home-mirror startup-scan fixes addressed locally; gateway sync regressions `82/82` and sync-client daemon guards `8/8` pass, alongside the earlier gateway peer-registry + home-mirror `29/29`, gateway home-mirror security `15/15`, Docker device-flow + proxy-routing `13/13`, Docker orchestrator `24/24`, shell theme `39/39`, and Docker platform blocker regressions `71/71`; local Playwright verification is still blocked by missing Chromium on this machine)
