...
- [ ] T008 Write failing schema/migration tests for immutable positive snapshot image generations, builds, durable rollout controls, durable revoked-base-generation markers, bounded immutable audit events, test-mode isolation/TTL, one-intent-per-lease uniqueness, stable pre-adoption recovery lease identity, snapshot-domain cleanup, existing `provider_deletion_queue` customer-clone cleanup integration, and provisioning-step tables/columns in `tests/platform/golden-snapshot-repository.test.ts`
...
- [ ] T011 Add Kysely table interfaces, generation-aware check/unique/index migrations, durable compatibility-scoped rollout controls, one-intent-per-lease plus provisioning-job intent linkage, bounded immutable audit-event retention, and provisioning job additions in `packages/platform/src/db.ts`
...
- [ ] T014 Implement advisory-lock-serialized generation allocation plus idempotent snapshot/build enqueue with `ON CONFLICT`, immutable release-provenance verification, a revoked-base-generation deny check, and audit-event insertion in the same lifecycle transaction in `packages/platform/src/golden-snapshot-repository.ts`
- [ ] T015 Implement advisory-lock-serialized capacity claims and validation-create reservations with a default-2/bounded-1..10 actual builder/validator resource cap, conditional phase advancement, attempt budgets, and lease expiry in `packages/platform/src/golden-snapshot-repository.ts`
...
- [ ] T018 Implement protected-reference/rollback/lease-aware retention, requiring a newer ready same-digest generation before freshness-expired protected retirement, and configured-cap enforcement in `packages/platform/src/golden-snapshot-repository.ts`
- [ ] T019 Add bounded disabled-by-default snapshot, compatibility, retry, lease, retention, max-concurrent-build, and rollout bootstrap config parsing in `packages/platform/src/customer-vps-config.ts`; initialize missing durable rollout rows with `ON CONFLICT DO NOTHING` and keep Postgres authoritative thereafter
...
- [ ] T033 [US2] Implement phase-bound token hashing, constant-time callback consumption, per-event `ON CONFLICT` callback receipts that retain and revalidate the accepted token digest plus bounded replay retention, and bounded evidence persistence in `packages/platform/src/golden-snapshot-service.ts`
...
**Checkpoint**: The 3a-3c image, orchestration, and build-control layers can produce a ready image under fakes, exercise the full fake-provider wiring path, never select it for a real customer, and never create production resources during tests.
...
- [ ] T046 [US1] Re-resolve current owner authorization and billing entitlement immediately before every billable provider create, including snapshot, clean-image fallback, and recovery retries; atomically create or reuse one snapshot intent per lease under the durable rollout/revocation lock; fail closed without a provider call when entitlement, rollout, snapshot, base generation, or intent was revoked; after every accepted or ambiguous create re-check durable intent denial before activation/routing; then pass exact provider snapshot IDs only after lease commit and preserve existing machine UUID labels in `packages/platform/src/customer-vps.ts`
...
- [ ] T059 [US3] Resume the same durable provisioning job through configured clean Ubuntu with fresh customer activation material, but re-resolve current owner authorization and billing entitlement immediately before the fallback provider create and every retry in `packages/platform/src/customer-vps-provisioning-jobs.ts`
...
- [ ] T063 [P] [US4] Write failing no-snapshot fallback, restore-failure, predecessor-routing-during-recovery, atomic routing handoff, and old-server cleanup tests in `tests/platform/golden-snapshot-recovery.test.ts`
...
- [ ] T065 [US4] Reuse atomic snapshot selection/lease for recovery while preserving backup preflight and ownership checks; allocate and lease the stable intended replacement UUID before provider create, then adopt that same UUID into `user_machines` without rotating the lease key while retaining the predecessor endpoint as the routable recovering projection in `packages/platform/src/customer-vps.ts`
...
- [ ] T067 [US4] Update `packages/platform/src/session-routing-middleware.ts` to route an authorized persisted `recovering` row to its retained predecessor endpoint until `packages/platform/src/customer-vps.ts` atomically adopts and routes the healthy replacement; preserve previous-server retirement and durable cleanup semantics for both snapshot and clean-image sources
...
- [ ] T069 [P] [US5] Write failing retention/quota/protected-channel/rollback/sole-compatible/lease and stale-generation replacement-handoff race tests in `tests/platform/golden-snapshot-retention.test.ts`
...
- [ ] T071 [P] [US5] Write failing immediate snapshot/base-generation revocation, atomic denial of overlapping create intents, durable pre-create/post-create/registration denial, future-enqueue/readiness denial, bounded generation batching, quarantine-during-active-lease plus high-priority cleanup after final release, cleanup retry, separate release/operator auth, and snapshot- plus generation-scoped affected-machine pagination tests in `tests/platform/golden-snapshot-repository.test.ts`
...
- [ ] T074 [US5] Implement retention, rollback/channel protection until a newer ready same-digest generation exists, only-compatible preservation, quota-pressure selection, and refusal when no safe deletion exists in `packages/platform/src/golden-snapshot-service.ts`
- [ ] T075 [US5] Implement immediate snapshot revocation plus an `ON CONFLICT` durable base-generation deny marker; deny overlapping create intents in the same transaction; require enqueue/readiness/selection/pre-create/post-create/registration checks; and follow with bounded deterministic snapshot/build cleanup batches in `packages/platform/src/golden-snapshot-repository.ts`
...
- [ ] T094 Restack and submit the eleven planned Graphite child PRs plus the separately scoped dev-validation child following `docs/dev/stacked-prs.md`, with source-of-truth, lock/transaction scope, acceptable orphan states, authorization source, and deferred scope in every PR body
...
| 2a | `feat(platform): define golden snapshot persistence` | T008-T009, T011-T012, T014-T019 | Migration-first Postgres/Kysely schema tests, repository, and disabled config; no provider I/O |
| 2b | `feat(platform): enforce golden snapshot lifecycle` | T010, T013, T020-T021 | Concurrency/selection tests and enforcement over the persistence parent |
| 3a | `feat(vps): prepare golden snapshot images` | T022, T025, T027-T032 and their T039 checks | Fake Hetzner contract plus packaged builder, sanitation, and validation scripts |
| 3b | `feat(vps): orchestrate golden snapshot builds` | T023, T026, T033-T036 | Fake-provider build state machine; image never ready before two-clone validation |
| 3c | `feat(platform): expose golden snapshot build controls` | T024, T037, T037A, T038-T039 | Authenticated bounded routes, fake-provider end-to-end wiring, worker shutdown, and route tests |
| 4a | `feat(platform): persist golden snapshot activation state` | Shared persistence from T044-T049 | Kysely job/provenance/lease/intent state with compatibility-safe callers |
| 4b | `feat(platform): provision VPSes from golden snapshots` | T040-T061 | Clone activation plus its customer orchestration callers, exact/older update, identity reset, and automatic clean-image fallback tests |
| 4c | `feat(platform): recover and retire golden snapshots` | T062-T078 | Recovery, retention, revocation, reconciliation, and bounded operations tests |
...
The concrete 2a/2b, 3a/3b/3c, and 4a/4b/4c boundaries keep every implementation PR
below 50 files and 3,000 additions. If later review fixes make a layer approach either
limit, stop and add another independently testable Graphite child before submission.
