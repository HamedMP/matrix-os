...
- [ ] T094 Restack and submit the twelve planned Graphite child PRs plus the separately scoped dev-validation child following `docs/dev/stacked-prs.md`, with source-of-truth, lock/transaction scope, acceptable orphan states, authorization source, and deferred scope in every PR body
...
| 2a | `feat(platform): define golden snapshot persistence` | T009, T011-T012, T014-T019 | Bounded Postgres/Kysely schema, repository, and disabled config; no provider I/O |
| 2b | `feat(platform): enforce golden snapshot lifecycle` | T008, T010, T013, T020-T021 | Migration/concurrency/selection tests over the persistence parent |
...
| 3c | `feat(platform): expose golden snapshot build controls` | T024, T037-T039 | Authenticated bounded routes, worker wiring, shutdown, and route tests |
...
| 4b | `feat(vps): activate golden snapshot clones` | T043-T044, T047-T050 | Clone cloud-init, exact/older activation, identity reset, and host tests |
| 4c | `feat(platform): provision VPSes from golden snapshots` | T040-T042, T045-T046, T049, T051-T061 | Customer clone orchestration and automatic clean-image fallback tests |
| 4d | `feat(platform): recover and retire golden snapshots` | T062-T078 | Recovery, retention, revocation, reconciliation, and bounded operations tests |
...
The concrete 2a/2b, 3a/3b/3c, and 4a/4b/4c/4d boundaries keep every implementation PR
