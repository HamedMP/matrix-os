...
2. **Given** snapshot creation is retried for the same bundle and compatibility class,
   **When** a candidate or freshness-eligible ready snapshot already exists, **Then**
   ...
   idempotent immutable replacement generation instead.
...
- **FR-004**: Repeated and concurrent requests for the same snapshot identity MUST be
  ...
  a deleted, retiring, or freshness-expired ready image. Generation allocation and
  creation MUST share the identity advisory lock, use `INSERT ... ON CONFLICT ... DO
  NOTHING`, and select/reuse the winning row in the same transaction. Check-then-insert
  and later deduplication of already-billable builds are forbidden.
...
- **FR-042**: Superseded snapshots MAY be deleted only when they are unreferenced,
  ...
  not either provider image generation.
...
- **Release-to-build path**: Host-bundle publication records the immutable release
  ...
  retiring, or freshness-expired identities. This repairs missed workflow requests and
  stale images without changing or blocking existing-fleet deployment.
