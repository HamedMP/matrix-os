...
2. **Given** snapshot creation is retried for the same bundle and compatibility class,
   **When** a candidate or freshness-eligible ready snapshot already exists, **Then**
   the process resumes or reuses the existing image generation rather than creating a
   duplicate. A deleted, retiring, or freshness-expired ready generation receives one
   idempotent immutable replacement generation instead. A quarantined generation with
   no active lease, including one quarantined before it ever became ready, is first
   moved through guarded retirement and receives the same automatic replacement.
...
- **FR-004**: Repeated and concurrent requests for the same snapshot identity MUST be
  ...
  database MUST enforce a unique immutable bundle-digest, compatibility-class,
  test-mode, and positive image-generation identity. The latest generation is the
  canonical target for ordinary retries; a new generation is permitted only to replace
  a deleted, retiring, freshness-expired ready, or quarantined image that is eligible
  for immediate guarded retirement because it has no active lease. This includes a
  pre-ready quarantined generation for which no provisioning lease could ever have
  existed. Its retirement transition and replacement-generation allocation MUST share
  the identity advisory lock. Generation allocation and creation MUST use `INSERT ...
  ON CONFLICT ... DO NOTHING` and select/reuse the winning row in the same transaction.
  Check-then-insert and later deduplication of already-billable builds are forbidden.
...
- **FR-010**: The system MUST distinguish candidate, building, sanitizing, validating,
  ...
  revocation MAY bypass channel, rollback, compatibility, and freshness protection.
  Any quarantined snapshot with no active lease, including a candidate, building,
  sanitizing, or validating snapshot that never became selectable, MUST be moved by the
  bounded reconciler through `quarantined -> retiring -> deleted` without waiting for a
  lease-release event. Before `retiring`, every owned builder, validator, or captured
  image resource MUST be durably queued by exact provider identity for cleanup.
  A terminal `failed` generation MAY be reset only by the bounded authenticated retry;
  otherwise retention MUST move it through `failed -> retiring -> deleted` after any
  owned provider resource is queued for exact cleanup. An eligible request arriving
  once it is `retiring` or `deleted` allocates one next immutable generation, preventing
  an exhausted build from permanently blocking that bundle and compatibility class.
  A snapshot entering `retiring` MUST be immediately
...
- **FR-021**: Before creating the VPS, the system MUST resolve the exact target host
  ...
  request. Builder and validation readiness MUST also exercise the baked AWS CLI through
  credential-free S3-compatible HEAD and download operations, while real primary-storage
  endpoint capability remains independently gated without exposing customer credentials
  to synthetic hosts. Missing or mismatched evidence MUST fall back to the full bootstrap.
...
- **FR-037**: Retries MUST be bounded, lease-based, and safe across worker crashes and
  ...
  keeping revoked images indefinitely. A quarantined snapshot that has no lease MUST
  be discovered and retired by the bounded reconciler directly; retirement MUST NOT
  depend on a lease having existed.
...
- **FR-042**: Superseded snapshots MAY be deleted only when they are unreferenced,
  ...
  fallback. Freshness expiry makes a row non-selectable and MUST proactively enqueue an
  immutable replacement generation for the same bundle digest and compatibility class.
  A stale channel- or rollback-protected row MUST remain available for exact-resource
  cleanup protection until that newer generation is validated `ready`; only then does
  an atomic selection handoff remove the old row's channel, rollback, and sole-fallback
  protection. Once the old row has no active lease, reconciliation MUST move it through
  the ordinary guarded retirement lifecycle rather than leave a permanently protected
  but unselectable `ready` row. The channel continues to reference the bundle digest,
  not either provider image generation. The same bounded retention pass MUST discover
  every quarantined row, including pre-ready rows with no provider image yet, queue all
  exact owned resources for cleanup, and move an unleased row through guarded
  retirement so it cannot block replacement generation allocation.
...
- **FR-045**: Snapshot-based provisioning MUST have an operator-controlled rollout
  switch and compatibility-scoped rollout percentage persisted in Postgres as the
  authoritative control plane for every platform instance. Environment configuration
  MAY provide only a bounded initial default when no durable control exists; it MUST
  NOT override an existing row. Selection and provider create MUST be separated by a
  durable create intent recorded under the same compatibility/generation advisory lock
  used by an authenticated operator rollout mutation. Disabling rollout under that
  lock MUST atomically update the durable control and deny every uncompleted intent
  that has not become customer-serving, immediately restore the clean-system-image
  path, and leave already running VPSes unchanged. Selection and intent validation MUST
  read the durable control inside that same lock so a stale process-local value cannot
  authorize a create. A job MUST re-read both the durable switch and its intent
  immediately before create and re-check the durable intent after every accepted or
  ambiguous create; a denied overlapping create MUST be isolated and exactly cleaned
  up and MUST NOT be activated or routed.
...
- **FR-054**: Operators MUST be able to revoke a snapshot or an entire base-system
  ...
  bounded inventories scoped either to one snapshot or to an entire base-system
  generation for prioritized in-place update, recovery, or other operator-directed
  remediation.
- **FR-055**: The system MUST support rebuilding the same host bundle against a newer
  approved base-system generation, and rebuilding it as a new image generation within
  the same compatibility class when freshness expires, and MUST enforce a bounded
  snapshot-freshness policy during selection.
...
| `GET /system-bundles/snapshots/:snapshotId/affected-machines?limit=1..100&cursor=<opaque>` | authorized operator automation | separate `GOLDEN_SNAPSHOT_OPERATOR_SECRET` | No |
...
| `PUT /system-bundles/snapshot-rollout` | authorized operator automation | separate `GOLDEN_SNAPSHOT_OPERATOR_SECRET` | No |
...
revocation and rollout mutation. The existing provision/recovery routes retain their
existing bounded-body middleware. Cleanup retry uses a 1 KiB body limit. Snapshot
identifiers or provider metadata never grant authority by themselves.
...
request.
Snapshot- and generation-scoped affected-machine inventories MUST apply the same
maximum page size and bounded server-issued opaque cursor, order deterministically by
retained machine identity, and return an optional next cursor. They MUST NOT materialize
the complete affected fleet in one request.
...
- **Release-to-build path**: Host-bundle publication records the immutable release
  ...
  bounded durable release reconciler owns missing/freshness detection: it discovers
  eligible releases with neither a freshness-eligible ready image nor an active build,
  and idempotently enqueues the next immutable generation for absent, deleted,
  retiring, freshness-expired, or quarantined identities. For a quarantined identity,
  it first queues every exact owned provider resource and atomically advances the
  unleased row to `retiring` under the identity advisory lock; a never-ready row does
  not wait for a nonexistent lease-release event. This repairs missed workflow requests,
  stale images, and pre-ready quarantine without changing or blocking existing-fleet
  deployment.
...
- **Config injection**: Snapshot/build enablement, compatibility defaults,
  retry/lease limits, and the bounded initial rollout default flow from environment-backed
  platform configuration into route, worker, and provisioning dependencies. The rollout
  switch and compatibility-scoped percentages are then read from and mutated in durable
  Postgres controls under the same advisory lock as selection/create intents; process-local
  environment values are never authoritative after initialization. A durable maximum of two concurrently
  snapshot infrastructure resources by default (bounded 1-10), provider timeouts, and callback origin
  also flow from bounded configuration. The worker computes capacity from each actual builder and
  validation-clone resource recorded by durable running builds, plus every cleanup-pending
  builder or validator whose exact absence is not yet confirmed, and reserves only the
  remaining slots inside one advisory-lock transaction (or an equivalent targeted atomic
  statement) before every provider create, so concurrent workers and restarts cannot
  exceed the configured infrastructure budget. Counting one running build as one slot
  when it owns both a builder and validator is forbidden, as is a read-then-create
  sequence outside that serialization boundary. Provider credentials remain server-side and are never
