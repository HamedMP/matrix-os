...
One immutable image-generation row per bundle digest and compatibility key. The latest
generation is the canonical enqueue target; older generations remain addressable for
leases, retirement, reconciliation, and exact-resource cleanup.
...
| `image_generation` | integer | Positive monotonic generation within bundle digest + compatibility + test-mode identity |
...
- Unique `(bundle_sha256, compatibility_key, test_mode, image_generation)` provides idempotent immutable generations without allowing an operator test candidate to satisfy a production request. Generation allocation is serialized under the same advisory lock as enqueue; ordinary retries reuse the latest row, while deleted, retiring, or freshness-expired ready rows may produce exactly one next generation.
...
## GoldenSnapshotRolloutControl

One durable row per compatibility key is the authoritative rollout control shared by
all platform instances.

| Field | Type | Rules |
|---|---|---|
| `compatibility_key` | 64-char hex | Primary key |
| `enabled` | boolean | Required; defaults false |
| `percentage` | integer | Required; bounded 0-100 |
| `generation` | bigint | Positive monotonic fencing value |
| `updated_at` | text | Required |

The bounded environment configuration is used only by `INSERT ... ON CONFLICT DO
NOTHING` to initialize a missing row. Operator mutation, selection, and create-intent
validation take the same compatibility advisory lock and read/update this row in the
same transaction. Disabling a row increments `generation` and denies every uncompleted
intent for that compatibility key before commit, so a platform process with stale
startup configuration cannot authorize a provider create.
...
| `builder_machine_id_sha256` | 64-char hex nullable | Persisted atomically from `builder_booted`; the later `sanitized` callback must match it exactly and must not overwrite it; compared with every validation clone after worker restart |
| `builder_ssh_host_key_sha256` | 64-char hex nullable | Persisted atomically from `builder_booted`; the later `sanitized` callback must match it exactly and must not overwrite it; compared with every validation clone after worker restart |
...
Because `provider_validation_id` stores only the validation clone currently being
reconciled, advancing `validation_clone_ordinal` from `1` to `2` must not orphan clone
1. In one Postgres transaction, the worker first inserts the exact clone-1 server ID
into the durable provider-cleanup queue with `INSERT ... ON CONFLICT`, then clears or
overwrites `provider_validation_id` and advances the ordinal. If cleanup enqueueing
fails, the transaction rolls back, leaving clone 1 as the current recoverable resource.
No provider call occurs while this transaction or its row lock is open; bounded cleanup
reconciliation uses the cleanup row as its source of truth after commit.
...
| `token_sha256` | 64-char hex | Digest of the phase token that authenticated the accepted event; replay must match in constant time |
...
on `(build_id, event_id)` makes a same-event retry return the stored outcome only when
both token and payload digests match in constant time, while token or payload drift fails
closed. Bounded pruning removes only expired receipts in immutable-key pages after the
replay window. Callback receipts intentionally retain only the payload digest and coarse
outcome, not identity evidence. The dedicated `GoldenSnapshotBuild` builder hash columns
are the crash-safe cross-phase source of truth: `builder_booted` writes them in the same
transaction as its receipt and phase transition, and `sanitized` compares both submitted
hashes to those locked columns before capture. A missing or different hash quarantines
the build; the sanitation callback never replaces the boot evidence.
...
The active association is enforced by a partial unique index on `machine_id` with
`WHERE released_at IS NULL`; released lease history remains non-unique so a machine
identity can be provisioned or recovered again after its prior lease is released.
...
| `snapshot_id`, `lease_id` | UUID text | FKs to the selected snapshot and active lease; `lease_id` is unique so one logical clone converges on one intent |
...
is no longer eligible. It uses `INSERT ... ON CONFLICT (lease_id) DO NOTHING` and then
selects the existing row; reuse succeeds only when snapshot, machine, purpose, and
rollout generation match exactly. The provisioning job stores that `intent_id`, making
worker retry and takeover resume the same durable intent rather than creating another
billable operation. Disabling rollout or revoking the snapshot/generation marks every
...
activation, and registration intent while retaining the predecessor routing endpoint.
The routing middleware treats only this persisted recovering projection as routable to
the predecessor until successful replacement registration atomically swaps the endpoint
and status. A crash before that claim has no provider side effect and reconciliation
releases the lease; a crash after it can resolve the lease through the durable recovering
row and exact replacement labels without interrupting predecessor routing.
...
- `snapshot_create_intent_id` (unique nullable FK; required for snapshot image source)
...
These fields make retries/resume explicit. The intent FK and the intent's unique lease
constraint ensure every retry resumes one create fence. Existing encrypted
registration/Postgres payload handling remains unchanged.
