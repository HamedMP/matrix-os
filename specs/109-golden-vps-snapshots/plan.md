...
host bundle publish
  ...
  -> idempotent enqueue attempt plus periodic missing-fresh/active-candidate reconciliation
...
candidate|building|sanitizing|validating -> quarantined (operator or base-generation revocation)
...
Unknown provider state is recorded on the build/cleanup job while the snapshot lifecycle remains non-selectable. A snapshot becomes `ready` only in the same transaction that stores successful validation evidence and clears the active build lease. Revocation atomically moves any nonterminal `candidate`, `building`, `sanitizing`, `validating`, or `ready` snapshot to `quarantined`. The authenticated retry endpoint is the only `failed -> candidate` path; it resets the bounded attempt budget and terminal fields in one transaction before the build becomes claimable again.
...
4. Reject candidates whose `ready_at` is older than the bounded configured freshness
   window. Release reconciliation proactively enqueues one next immutable image
   generation for the same bundle digest and compatibility class. A stale protected
   row enters guarded retirement only after the replacement validates `ready`, making
   the selection handoff atomic from the database's perspective.
...
Existing `provisioning_jobs` remains the durable owner-machine workflow. It gains explicit image-source, target-release, activation-step, snapshot-lease, and create-intent fields so restart recovery does not infer progress from the current machine row. A unique lease-to-intent constraint plus `ON CONFLICT` reuse makes overlapping workers converge on one provider-create fence. Snapshot selection and create-intent persistence are short transactions; clone creation is outside them. Exact provider labels use the existing immutable machine UUID, allowing reconciliation after a lost create response. Compatibility-scoped rollout controls are durable Postgres rows initialized once from bounded defaults; rollout disablement and revocation deny every overlapping uncompleted intent under the same advisory-lock order, and provisioning/recovery read that durable control and re-check denial immediately before every provider create and after every accepted or ambiguous response. During recovery, the persisted `recovering` row keeps routing to its predecessor endpoint until healthy replacement registration atomically swaps the route.
...
| `GET /system-bundles/snapshots/:snapshotId/affected-machines?limit=1..100&cursor=<opaque>` | Constant-time `GOLDEN_SNAPSHOT_OPERATOR_SECRET` bearer check | Operator only; bounded cursor-paginated coarse remediation inventory for one image |
...
| `PUT /system-bundles/snapshot-rollout` | Constant-time `GOLDEN_SNAPSHOT_OPERATOR_SECRET` bearer check | Operator only; mutates one durable compatibility-scoped rollout row and fences intents; 4 KiB bounded mutation |
...
All mutating routes use `bodyLimit`; params/bodies use strict bounded Zod schemas. Callback tokens are phase-bound, expire at the persisted external-phase `callback_expires_at` deadline independently of the shorter reclaimable worker lease, are cleared after one successful use, and never appear in URLs or logs. Accepted events persist bounded receipts keyed by `(build_id, event_id)` together with the accepted token digest for the replay window, so a later phase cannot erase an earlier retry outcome and an unauthenticated or wrong-token replay cannot recover that outcome. Provider/raw validation errors map to bounded internal reason codes and generic client responses.

### Transaction and lock scope

- `enqueueSnapshotBuild`: one transaction takes the bundle/compatibility/test-mode
  advisory lock, allocates or reuses the latest positive image generation, upserts the
  build job, and records audit/outbox state. `ON CONFLICT` is keyed by bundle digest +
  compatibility key + persisted test-mode isolation + image generation. Release
  registration is the durable source event, and a bounded periodic reconciler scans
  eligible releases without a fresh ready or active production candidate so missed,
  deleted, retiring, and freshness-expired snapshots receive one idempotent replacement.
- `claimSnapshotBuild` and validation-create reservation: short advisory-lock
  transactions count each durable builder and validator resource independently plus
  every unresolved cleanup row whose exact absence has not been confirmed, then claim
  or persist the next create intent only within remaining configured capacity. The cap
  defaults to 2 and is bounded to 1-10; one build owning a builder and validator consumes
  two slots. Provider calls occur only after commit.
...
- `createSnapshotIntent`: one short transaction takes the compatibility/generation and
  snapshot advisory locks, reads the durable rollout row, revalidates rollout plus
  snapshot eligibility, and inserts or reuses the unique lease-bound intent before
  provider create. Rollout mutation, snapshot/generation revocation, selection, and
  intent validation use the same lock order; disablement marks overlapping uncompleted
  intents denied before commit.
...
- `retire/delete`: one transaction marks non-selectability and queues exact cleanup; a
  ...
  protection after the final lease release. A freshness-expired channel/rollback image
  stays protected until a newer ready same-digest generation exists. Provider delete
  occurs later, followed by a conditional completion update.
...
2. `feat(platform): define golden snapshot persistence`
3. `feat(platform): enforce golden snapshot lifecycle`
4. `feat(vps): prepare golden snapshot images`
5. `feat(vps): orchestrate golden snapshot builds`
6. `feat(platform): expose golden snapshot build controls`
7. `feat(platform): persist golden snapshot activation state`
8. `feat(platform): provision VPSes from golden snapshots`
9. `feat(platform): recover and retire golden snapshots`
10. `ci(release): create snapshots for eligible host bundles`
11. `docs(vps): document snapshot operations and rollout`

These concrete boundaries keep every planned implementation PR below the repository's
3,000-addition and 50-file limits. The separately scoped dev-validation gate remains a
final child above the documentation layer.
