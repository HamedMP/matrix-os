...
## GET `/system-bundles/snapshots/:snapshotId/affected-machines`

Authenticated with `GOLDEN_SNAPSHOT_OPERATOR_SECRET`; `snapshotId` is a bounded UUID.
The query, cursor ordering, response fields, omissions, and generic error behavior are
identical to the base-generation inventory above, except rows are restricted to the
exact persisted `sourceSnapshotId`. This lets operators remediate one revoked or
suspect image without broadening the result to every image in its base generation.
...
## PUT `/system-bundles/snapshot-rollout`

Authenticated with `GOLDEN_SNAPSHOT_OPERATOR_SECRET`; the body is limited to 4 KiB
and parsed as a strict bounded schema:

```json
{
  "compatibilityKey": "64-hex",
  "enabled": false,
  "percentage": 0
}
```

The transaction takes the compatibility advisory lock, inserts or updates the durable
rollout row, increments its monotonic generation, and when disabling marks every
uncompleted create intent for that compatibility key denied before commit. Selection
and pre-create validation take the same lock and read this row. The response exposes
only `{ "updated": true, "enabled": false, "percentage": 0 }`; generation, intent
identities, owner data, and provider details remain internal. Errors are generic
`400/401/500`.
