...
- **Phase A — Provisioning reliability (US2)**: reconciliation covers booted-but-never-registered machines (registration token expired ⇒ failed); `failed` rows stop blocking provision (retire-on-retry inside the provision transaction); `attempt` counter (the one new `user_machines` column) + bounded retry; stale `/vps/register` rejection. (`recover()` was verified already atomic.) No UI change. The `billing_checkout_attempts` table + checkout recording + settling-aware derivation move to Phase B, shipping with their consumer.
- **Phase B — Journey core + settling (US1, US3)**: `journey.ts` derivation + `onboarding_journey_events` telemetry + `onboarding_first_run` + `billing_checkout_attempts` tables; `GET /api/journey`; `POST /api/journey/retry-provision`; `POST /internal/first-run`; `/billing/checkout` records an attempt and the Stripe `checkout.session.*` webhook resolves it; reconciler backfill for legacy users + stale-attempt sweep, both off the read path.
