...
2. Entitlement not active/trialing/grace AND no `open` checkout attempt → `plan_required`
3. Entitlement not yet active AND an `open` checkout attempt exists → `payment_settling` (annotated `settling.delayed = (age > settling window)`; the phase does **not** change when the window is exceeded — see R3)
...
**Decision**: Record a `billing_checkout_attempts` row when `/billing/checkout` creates a Stripe session (`billing-routes.ts:102-141`): `clerk_user_id`, `stripe_session_id`, `created_at`, `status (open|completed|expired|abandoned)`. The webhook marks it `completed`; `checkout.session.expired` (already received via the same webhook endpoint) marks it `expired`. Journey derivation reports `payment_settling` whenever an `open` attempt exists and entitlement is not yet active, regardless of attempt age. The settling window (default 10 min, env-tunable) only controls the `settling.delayed` annotation: within the window `delayed:false` (calm "activating your subscription" UI), past the window `delayed:true` with a `contact_support` next action (FR-014). The phase **never** silently drops to `plan_required` while an `open` attempt exists — that would re-show the paywall to a user who just paid, the exact regression US3 forbids. The user leaves `payment_settling` only when the entitlement activates (→ `provisioning`) or the attempt resolves to `expired`/`abandoned` via the Stripe webhook (→ `plan_required`). Settling is therefore device-independent and survives reloads; sessionStorage logic is deleted.
