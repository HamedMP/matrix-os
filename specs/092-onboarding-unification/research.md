...
2. Entitlement not active/trialing/grace AND there is neither a `paid` attempt nor an `open` attempt within the settling window → `plan_required` (covers no attempt, an abandoned/expired attempt, and an unconfirmed `open` attempt past the window)
3. Entitlement not yet active AND (a `paid` attempt exists, OR an `open` attempt within the settling window) → `payment_settling`. A `paid` attempt is sticky and never drops to `plan_required`; the window only sets `settling.delayed` (`= paid && age > window`). An `open` attempt that ages past the window stops sustaining settling and falls to rule 2 — see R3.
