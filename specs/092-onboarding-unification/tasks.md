...
- [ ] T027 [US1] Implement lazy backfill in `journey.ts`: machine `running` + no first-run record → one proxied gateway `onboarding-status` probe with `AbortSignal.timeout(3000)`; positive → persist record with `source='backfill'`; timeout/negative → phase stays `first_run` (R4)
