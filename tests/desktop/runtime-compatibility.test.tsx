...
describe("runtime compatibility lifecycle", () => {
  ...
  it("preserves a mounted draft through incompatibility and recovery", async () => {
    ...
    act(() => window.dispatchEvent(new Event(RUNTIME_RECONNECTED_EVENT)));
    await screen.findByText("Update Matrix OS Desktop to continue");
  ...
  it("reports legacy metadata without preventing existing features from loading", async () => {
    ...
    expect(screen.getByRole("status").textContent).toContain("does not report compatibility");
  });
