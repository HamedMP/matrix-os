...
describe("ShellSocket server frames", () => {
  ...
  it("keeps output flowing when resize reports a lower workspace revision than the tab", () => {
    const h = createHarness();
    h.socket.connect();
    h.latest().open();
    h.latest().frame({ type: "attached", revision: 9, nextSeq: 0 });
    h.latest().frame({
      type: "canonical-size",
      revision: 3,
      canonicalSize: { cols: 132, rows: 36 },
    ...
    h.latest().frame({ type: "output", revision: 3, seq: 1, data: "still live" });
    h.latest().frame({ type: "exit", revision: 4, code: 0 });

    expect(h.events.outputs).toEqual([{ data: "still live", seq: 1 }]);
    expect(h.events.exits).toEqual([0]);
  ...
  it("accepts an ordinary snapshot after synthetic live-tail output", () => {
    const h = createHarness();
    connectAndAttach(h);
    h.latest().frame({
      type: "output",
  ...
  it("does not append a repeated live-tail sequence after reconnect", () => {
    const h = createHarness();
    connectAndAttach(h);
    h.latest().frame({
      type: "output",
    ...
    expect(h.latest().url).toContain(`fromSeq=${LIVE_TAIL_FROM_SEQ}`);
    h.latest().open();
    h.latest().frame({ type: "attached", revision: 2, nextSeq: LIVE_TAIL_FROM_SEQ });
    h.latest().frame({
      type: "output",
      revision: 2,
      seq: LIVE_TAIL_FROM_SEQ,
    ...
    expect(h.events.outputs).toEqual([
      { data: "live tail once", seq: LIVE_TAIL_FROM_SEQ },
    ]);
  });

  it("preserves the resume watermark after an authoritative replacement snapshot", () => {
    const h = createHarness();
    connectAndAttach(h);
  ...
  it("resets to a replacement presentation when retained replay is incomplete", () => {
    const h = createHarness();
    connectAndAttach(h);
    ...
    h.latest().frame({ type: "output", revision: 3, seq: 8, data: "new-presentation output" });
    expect(h.events.outputs.at(-1)).toEqual({ data: "new-presentation output", seq: 8 });
    expect(h.socket.lastSeq).toBe(8);
    expect(h.sockets).toHaveLength(2);
  });
