...
describe("Chat Agent mention admission contracts", () => {
  it("stores an immutable Agent identity on supported Runs without admitting client-forged snapshots", () => {
    ...
    expect(CanonicalChatRunSchema.safeParse({ ...run, driverKind: "codex", context }).success).toBe(true);
    expect(CanonicalChatRunSchema.safeParse({ ...run, driverKind: "pi", context }).success).toBe(false);
