...
const pluginRoot = join(root, "plugins/matrix-onboarding");
...
describe("Matrix OS Codex marketplace plugin", () => {
  it("preserves the compatibility identifier while presenting one Matrix OS product", () => {
    ...
    expect(marketplace.plugins?.[0]).toMatchObject({
      ...
      source: { path: "./plugins/matrix-onboarding" },
      category: "Productivity",
    });
    expect(dirname(dirname(manifestPath)).split("/").at(-1)).toBe("matrix-onboarding");
  ...
  it("documents bounded command execution and sandboxed coding sessions", () => {
    ...
    expect(skill).toContain("matrix run --json -C <dir> -- <argv...>");
    expect(skill).toMatch(/exitCode/);
    expect(skill).toMatch(/timedOut/);
    expect(skill).toMatch(/truncated/);
    expect(skill).toMatch(/unique[^\n]*session/i);
    expect(skill).toMatch(/matrix shell connect <session-name>/);
