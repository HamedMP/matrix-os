...
describe("TerminalApp workspace contract", () => {
  ...
  it("hands off a queued command that finishes creating after unmount without deleting or rerunning it", async () => {
    ...
    expect(vi.mocked(fetch).mock.calls.some(([input, init]) => (
      String(input).endsWith(`/api/terminal/workspaces/${WORKSPACE_ID}/tabs/${SECOND_TAB_ID}`)
      && init?.method === "DELETE"
    ))).toBe(false);
  });

  it("hands off a late-created launch when storage persistence fails", async () => {
    let resolveTabResponse: ((response: Response) => void) | undefined;
    const tabResponse = new Promise<Response>((resolve) => {
      resolveTabResponse = resolve;
    });
  ...
  it("restores a canonical handoff when its receiving terminal unmounts during discovery", async () => {
    const view = render(<TerminalApp initialSessionId={REF_KEY} launchTargetId="runtime-settings" />);
    await settle();
    ...
    vi.mocked(fetch).mockImplementation(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith("/api/terminal/workspaces")) return discovery;
      if (url.endsWith("/api/terminal/preferences")) return json({ preferences: {} });
      if (url.includes("/api/terminal/layout")) return json({});
      if (url.endsWith("/api/agents")) return json({ agents: [] });
      if (url.endsWith("/api/files/tree")) return json([]);
    ...
    enqueueExistingTerminalRef(`${WORKSPACE_ID}:${SECOND_TAB_ID}`, "runtime-settings");
    await act(async () => { await Promise.resolve(); });
    view.unmount();
    await act(async () => {
      resolveDiscovery?.(Response.json({ workspaces: [workspace([tab(), tab(SECOND_TAB_ID)])] }));
      await Promise.resolve();
      await Promise.resolve();
    });

    await expect(drainExistingTerminalSessionQueue(undefined, {
      fetcher: async () => Response.json({ workspaces: [workspace([tab(), tab(SECOND_TAB_ID)])] }),
    })).resolves.toEqual([`${WORKSPACE_ID}:${SECOND_TAB_ID}`]);
  });
