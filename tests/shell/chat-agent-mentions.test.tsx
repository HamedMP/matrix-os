...
it("starts a fresh New Chat after abandoning a draft with references", async () => {
  const submit = vi.fn(async () => false);
  function Workspace() {
    const [sessionId, setSessionId] = React.useState<string | undefined>(undefined);
    return <ChatApp {...base} sessionId={sessionId} onSubmit={submit} agentClient={client}
      conversations={[{ id: "chat_original", title: "Saved Chat", preview: "", messageCount: 1, updatedAt: 1000 }]}
      onNewChat={() => setSessionId(undefined)} onSwitchConversation={setSessionId} />;
  }
  render(<Workspace />);
  const editor = () => screen.getByRole("textbox", { name: "Message chat" }) as HTMLTextAreaElement;
  await waitFor(() => expect(editor().disabled).toBe(false));
  fireEvent.change(editor(), { target: { value: "@mee" } });
  fireEvent.click(await screen.findByRole("option", { name: /Meeting notes/ }));
  fireEvent.change(editor(), { target: { value: "Abandoned new Chat" } });
  fireEvent.click(screen.getByRole("button", { name: "Saved Chat" }));
  fireEvent.change(editor(), { target: { value: "Saved Chat draft" } });
  fireEvent.click(screen.getByTitle("New chat"));
  expect(editor().value).toBe("");
  expect(screen.queryByRole("button", { name: "Remove Meeting notes" })).toBeNull();
  fireEvent.change(editor(), { target: { value: "Another abandoned draft" } });
  fireEvent.click(screen.getByTitle("New chat"));
  expect(editor().value).toBe("");
  fireEvent.click(screen.getByRole("button", { name: "Saved Chat" }));
  expect(editor().value).toBe("Saved Chat draft");
});

it("keeps the first website recipe handoff and rejected new Chat draft until explicitly replaced", async () => {
  const submit = vi.fn(async () => false);
  function Workspace() {
    const [draft, setDraft] = React.useState<{ id: number; text: string; resources: typeof source[] } | null>({
      id: 1, text: "Website recipe handoff", resources: [source],
    });
    return <ChatApp {...base} sessionId={undefined} messages={[]} onSubmit={submit} agentClient={client}
      composerDraftRequest={draft} onComposerDraftConsumed={() => setDraft(null)} />;
  }
  render(<Workspace />);
  const editor = () => screen.getByRole("textbox", { name: "Message chat" }) as HTMLTextAreaElement;
  await waitFor(() => expect(editor().disabled).toBe(false));
  expect(editor().value).toBe("Website recipe handoff");
  fireEvent.click(screen.getByRole("button", { name: "Send" }));
  ...
  expect(submit.mock.calls[1]![2]?.clientRequestId).toBe(submit.mock.calls[0]![2]?.clientRequestId);
  fireEvent.click(screen.getByTitle("New chat"));
  expect(editor().value).toBe("");
  expect(screen.queryByRole("button", { name: "Remove Meeting notes" })).toBeNull();
});

it("preserves the existing text-only busy composer behavior without treating text as new reference work", async () => {
  const submit = vi.fn();
  render(<ChatApp {...base} busy onSubmit={submit} agentClient={client} />);
  const editor = screen.getByRole("textbox", { name: "Message chat" });
  await waitFor(() => expect((editor as HTMLTextAreaElement).disabled).toBe(false));
  fireEvent.change(editor, { target: { value: "Ordinary text while running" } });
...
it("requires fresh consent when a website handoff replaces a draft with the same Agent", async () => {
  ...
  function Workspace() {
    const [draft, setDraft] = React.useState<{ id: number; text: string; resources: typeof agent[] } | null>({
      id: 1, text: "First recipe", resources: [agent],
    });
    return <>
      <button onClick={() => setDraft({ id: 2, text: "Replacement recipe", resources: [agent] })}>Replace recipe</button>
      <ChatApp {...base} sessionId={undefined} messages={[]} onSubmit={submit} agentClient={client}
        composerDraftRequest={draft} onComposerDraftConsumed={() => setDraft(null)} />
    </>;
  }
  render(<Workspace />);
  ...
  await waitFor(() => expect(editor().disabled).toBe(false));
  fireEvent.click(screen.getByRole("checkbox", { name: /Allow Full access/ }));
  fireEvent.change(editor(), { target: { value: "First recipe with my details" } });
