...
it("hydrates a recipe draft without restoring a previous Chat or sending a request", async () => {
  ...
  expect(fetcher.mock.calls.every(([url]) => !url.includes("/turns"))).toBe(true);
  const id = result.current.composerDraftRequest!.id;
  act(() => result.current.consumeComposerDraft!(id));
  rerender({ prompt: "Prepare a synthetic weekly report" });
...
it("uses the shared automatic title without shortening the actual request", async () => {
  ...
  expect(creates[0]!.title).toBe("Personal Daily Brief: Review today's messages and…");
