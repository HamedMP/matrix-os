...
async function waitForLog(
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (latestLog.includes(expected)) return latestLog;
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(
        `Standalone daemon exited before logging ${JSON.stringify(expected)}:\n${latestLog}`,
      );
    }
...
async function expectHappyPathDaemonStartsOnce(options: {
  ...
  try {
    ...
    expect(await readFile(join(configDir, "daemon.pid"), "utf8")).toBe(String(daemon.pid));
  } finally {
