...
describe("sync routes: userId resolution from JWT", () => {
  ...
  it("tampered JWT falls through to legacy bearer path and uses MATRIX_HANDLE", async () => {
    ...
    // Flip the first character of the signature segment. Base64url uses
    // [A-Za-z0-9_-]; swap 'A' ↔ 'B' so the token still *looks* like a JWT
    // (3 base64url segments) and reaches the validator. Do not mutate the
    // final base64url character: for HS256 signatures it can include unused
    // padding bits and may decode back to the same bytes.
