...
describe("gateway project inventory source", () => {
  ...
  it("rejects a session cwd that escapes the configured home before project containment", async () => {
    const current = dependencies();
    current.sessions.list = vi.fn(async () => [{
      name: "terminal-owned",
      canonicalName: "terminal-owned",
    ...
    await expect(createGatewayProjectInventorySource(current).listTerminals(OWNER, PROJECT))
      .resolves.toContainEqual(expect.objectContaining({
        id: "terminal-owned",
        compatibility: "blocked",
        blocker: "terminal_isolation_unavailable",
      }));
  });
});
