...
describe("shared Chat canonical queue", () => {
  ...
  it("accepts work while idle or busy and keeps one active run", async () => {
    ...
    const second = await repository.enqueueSharedQueuedTurn(owner, request(2, collaborationActors.editor));
  ...
  it("scopes request idempotency to the actor and rejects payload substitution", async () => {
    ...
    const otherActor = await repository.enqueueSharedQueuedTurn(owner, request(10, collaborationActors.editor));
  ...
  it("allows 32 pending requests and rejects the thirty-third without consuming order", async () => {
    for (let index = 1; index <= 32; index += 1) {
      await expect(repository.enqueueSharedQueuedTurn(
        owner,
        request(index, index % 2 === 0 ? collaborationActors.editor : collaborationActors.owner),
      )).resolves.toMatchObject({ acceptedSequence: index, pendingCount: index });
    }

    await expect(repository.enqueueSharedQueuedTurn(owner, request(33, collaborationActors.editor)))
...
function request(index: number, actorId: string) {
  return {
    chatId: collaborationIds.chat,
