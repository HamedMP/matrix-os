...
describe("CanvasService", () => {
  ...
  it("delegates terminal actions to the project workspace runtime", async () => {
    ...
    const service = new CanvasService(repository([record()]), {
      ...
      homePath,
      resolveProjectWorkingDirectory: vi.fn().mockResolvedValue(projectPath),
    });
    ...
    expect(runtime.createTab).toHaveBeenCalledWith(workspaceId, { name: "terminal", cwd: projectPath });
  });
  ...
  it("rejects terminal cwd outside the canvas project working directory", async () => {
    const homePath = await mkdtemp(join(tmpdir(), "canvas-home-"));
    const projectPath = join(homePath, "projects", "app");
    await mkdir(projectPath, { recursive: true });
