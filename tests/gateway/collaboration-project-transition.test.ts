...
describe("project collaboration transition journal", () => {
  ...
  it("replays a completed preparation before callers rebuild an expiring inventory", async () => {
    const transitions = journal();
    await prepare();

    await expect(transitions.replayPreparation({
      scopeId: SCOPE_ID,
      requestedBy: OWNER_ID,
      clientRequestId: CLIENT_REQUEST_ID,
      payloadHash: PAYLOAD_HASH,
    })).resolves.toMatchObject({ id: TRANSITION_ID, status: "prepared" });
    await expect(transitions.replayPreparation({
      scopeId: SCOPE_ID,
      requestedBy: OWNER_ID,
      clientRequestId: CLIENT_REQUEST_ID,
