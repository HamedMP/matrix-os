...
describe("createGateway Chat terminal production wiring", () => {
  it("keeps createGateway on the workspace/tab runtime without constructing a legacy Zellij registry", () => {
    ...
    expect(source).toContain("createTerminalTabWebSocketHandler(c, {");
    const socketSource = readFileSync(join(process.cwd(), "packages/gateway/src/shell/terminal-tab-ws.ts"), "utf8");
    expect(socketSource).toContain("const refAccess = await terminalRuntimeRefAccess(");
    expect(socketSource).toContain('refAccess === "chat_required"');
    expect(socketSource).toContain('refAccess === "repository_required"');
    const workspaceSocket = socketSource.indexOf("export function createTerminalTabWebSocketHandler");
    const chatAuthorization = socketSource.indexOf("if (chatResult.data)", workspaceSocket);
    const repositoryGuard = socketSource.indexOf("if (!terminalAuthorizationRepository)", chatAuthorization);
    const ownerOnlyFallback = socketSource.indexOf("if (terminalAuthorizationRepository)", repositoryGuard);
    ...
    expect(ownerOnlyFallback).toBeLessThan(socketSource.indexOf("runtime.attach({", workspaceSocket));
    const workspaceAdmission = socketSource.indexOf("projectAdmission.withWorkspace(", workspaceSocket);
    const attach = socketSource.indexOf("runtime.attach({", workspaceSocket);
    const frameAdmission = socketSource.indexOf("projectAdmission.withWorkspace(", workspaceAdmission + 1);
    const frameSend = socketSource.indexOf("stream.send(frame)", frameAdmission);
