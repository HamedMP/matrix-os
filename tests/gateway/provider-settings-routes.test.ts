...
describe("provider settings routes", () => {
  it("negotiates atomic connection support without changing legacy snapshot responses", async () => {
    const enabledIntent: ProviderSettingsSnapshot = {
      ...snapshot,
      accessSources: [{ ...snapshot.accessSources[0]!, localObservation: {
        state: "present_unverified", checkedAt: "2026-08-30T10:00:00.000Z", staleAfter: "2026-08-30T10:00:05.000Z",
      } }],
      harnesses: [{
