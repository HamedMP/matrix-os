...
describe("customer VPS Symphony systemd unit", () => {
  ...
  it("provisions Erlang runtime with Symphony installed but idle", async () => {
    ...
    expect(cloudInit).toContain("systemctl enable matrix-restore.service matrix-gateway.service matrix-shell.service matrix-code-server.service matrix-code.service matrix-sync-agent.service");
    expect(cloudInit).toContain("systemctl start matrix-restore.service matrix-gateway.service matrix-shell.service matrix-sync-agent.service");
