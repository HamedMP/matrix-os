...
describe("customer VPS Symphony systemd unit", () => {
  ...
  it("enables and starts bundled Symphony units during existing VPS updates", async () => {
    ...
    expect(syncAgent).toContain('if sudo systemctl cat matrix-terminal-runtime.service >/dev/null 2>&1; then');
    expect(syncAgent).toContain('if sudo systemctl cat matrix-scope-runtime.service >/dev/null 2>&1; then');
    expect(syncAgent).toContain('if ! sudo systemctl stop "${runtime_services[@]}"; then');
    expect(syncAgent).toContain('sudo systemctl show --property=ActiveState --value "$service"');
    expect(syncAgent).not.toContain("sudo systemctl stop matrix-symphony matrix-gateway matrix-shell matrix-scope-runtime");
  });
