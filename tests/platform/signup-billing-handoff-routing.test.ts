...
describe("platform signup billing handoff routing", () => {
  it("recognizes only the exact bounded marker", () => {
    ...
    expect(isSignupBillingHandoff(
      "/sign-in?billing=setup&handoff=signup",
    )).toBe(false);
    expect(isSignupBillingHandoff(
  ...
  it("allows only the existing signup showcase assets through the platform namespace", () => {
    ...
    expect(
      getPlatformShellAssetUpstreamPath("/__platform-shell/rabbit.svg"),
    ).toBeNull();
    expect(
