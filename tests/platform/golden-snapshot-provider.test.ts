...
describe('golden snapshot Hetzner provider contract', () => {
  ...
  it('accepts the transient deleting image state used by cleanup reconciliation', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(new Response(JSON.stringify({ image: {
      id: 301, status: 'deleting', type: 'snapshot', architecture: 'x86', disk_size: 40,
      labels: {}, protection: { delete: false },
    } }), { status: 200 }));
    const client = createHetznerClient(config, fetchImpl as unknown as typeof fetch);

    await expect(client.getImage(301)).resolves.toMatchObject({ id: 301, status: 'deleting' });
  });

  it('treats a successful exact-id response with a null image as absent', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ image: null }), { status: 200 }),
    );
    const client = createHetznerClient(config, fetchImpl as unknown as typeof fetch);

    await expect(client.getImage(301)).resolves.toBeNull();
  });
