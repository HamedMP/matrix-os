...
import {
  ...
  getGoldenSnapshotRolloutControl,
...
describe('golden snapshot repository', () => {
  ...
  beforeEach(async () => {
    ...
    await initializeGoldenSnapshotRolloutControl(db, {
      compatibility,
      enabled: true,
      percentage: 100,
  ...
  it('migrates every authoritative lifecycle table', async () => {
    ...
    expect(rows.map((row) => row.table_name)).toEqual([
      ...
      'golden_snapshot_rollout_controls',
      'golden_snapshots',
    ]);

    const receiptColumns = await db.executor
      .selectFrom('information_schema.columns')
      .select(['column_name', 'is_nullable'])
      .where('table_schema', '=', 'public')
      .where('table_name', '=', 'golden_snapshot_callback_receipts')
      .orderBy('ordinal_position')
      .execute();
    expect(receiptColumns.map((row) => row.column_name)).toContain('token_sha256');
    expect(receiptColumns.find((row) => row.column_name === 'token_sha256')?.is_nullable).toBe('YES');

    const snapshotColumns = await db.executor.selectFrom('information_schema.columns')
      .select('column_name').where('table_schema', '=', 'public')
      .where('table_name', '=', 'golden_snapshots').execute();
    expect(snapshotColumns.map((row) => row.column_name)).toContain('image_generation');
  });

  it('persists rollout authority once and fences overlapping create intents on mutation', async () => {
    await expect(initializeGoldenSnapshotRolloutControl(db, {
      compatibility,
      enabled: false,
      percentage: 0,
      now: '2026-07-03T00:00:00.000Z',
    })).resolves.toMatchObject({ enabled: true, percentage: 100, generation: 1 });

    const enqueued = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000063',
      buildId: '20000000-0000-4000-8000-000000000063',
      now: '2026-07-03T00:01:00.000Z',
    });
    await db.executor.updateTable('golden_snapshots').set({
      state: 'ready', provider_image_id: 963, provider_image_status: 'available',
      image_disk_gb: 40, image_architecture: 'x86', ready_at: '2026-07-03T00:02:00.000Z',
    ...
    const selected = await selectAndLeaseGoldenSnapshot(db, {
      ...
      machineId: '30000000-0000-4000-8000-000000000063', purpose: 'provision',
      leaseId: '40000000-0000-4000-8000-000000000063',
      now: '2026-07-03T00:03:00.000Z', expiresAt: '2026-07-03T00:13:00.000Z',
    });
    expect(selected).toBeDefined();
    await expect(createGoldenSnapshotCreateIntent(db, {
      intentId: '50000000-0000-4000-8000-000000000063',
      snapshotId: enqueued.snapshot.snapshotId,
      leaseId: selected!.lease.leaseId,
      machineId: selected!.lease.machineId,
      purpose: 'provision', rolloutGeneration: 1,
      now: '2026-07-03T00:04:00.000Z',
    })).resolves.toMatchObject({ state: 'pending', rolloutGeneration: 1 });

    await expect(updateGoldenSnapshotRolloutControl(db, {
      compatibilityKey: enqueued.snapshot.compatibilityKey,
      enabled: false,
      percentage: 0,
      now: '2026-07-03T00:05:00.000Z',
    })).resolves.toMatchObject({ enabled: false, percentage: 0, generation: 2 });
    await expect(getGoldenSnapshotCreateIntent(db, selected!.lease.leaseId))
      .resolves.toMatchObject({ state: 'denied' });
    await expect(getGoldenSnapshotRolloutControl(db, enqueued.snapshot.compatibilityKey))
      .resolves.toMatchObject({ enabled: false, percentage: 0, generation: 2 });
  });

  it('creates a new immutable image generation after the canonical snapshot was deleted', async () => {
    const first = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000060',
      buildId: '20000000-0000-4000-8000-000000000060', now: '2026-07-03T00:00:00.000Z',
    });
    await db.transaction(async (trx) => {
      await trx.executor.updateTable('golden_snapshot_builds').set({
        phase: 'completed', status: 'completed', completed_at: '2026-07-03T00:01:00.000Z',
      }).where('build_id', '=', first.build.buildId).execute();
      await trx.executor.updateTable('golden_snapshots').set({
        state: 'deleted', deleted_at: '2026-07-03T00:01:00.000Z',
      }).where('snapshot_id', '=', first.snapshot.snapshotId).execute();
    });

    const replacement = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000061',
      buildId: '20000000-0000-4000-8000-000000000061', now: '2026-07-03T00:02:00.000Z',
    });
    expect(replacement).toMatchObject({
      reused: false,
      snapshot: { snapshotId: '10000000-0000-4000-8000-000000000061', imageGeneration: 2 },
      build: { buildId: '20000000-0000-4000-8000-000000000061', status: 'queued' },
    });
    await expect(enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000062',
      buildId: '20000000-0000-4000-8000-000000000062', now: '2026-07-03T00:03:00.000Z',
    })).resolves.toMatchObject({
      reused: true,
      snapshot: { snapshotId: replacement.snapshot.snapshotId, imageGeneration: 2 },
      build: { buildId: replacement.build.buildId },
    });
  });
  ...
  it('counts each unresolved provider server instead of one row per running build', async () => {
    const occupied = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
    ...
    await enqueueGoldenSnapshotBuild(db, {
      ...
      buildId: '20000000-0000-4000-8000-000000000044', now: '2026-07-03T00:00:01.000Z',
    });
    await db.executor.updateTable('golden_snapshot_builds').set({
      status: 'running', phase: 'validation_boot', attempts: 1,
      lease_expires_at: '2026-07-03T00:10:00.000Z', callback_expires_at: '2026-07-03T00:30:00.000Z',
      provider_builder_id: 943, provider_validation_id: 944,
    }).where('build_id', '=', occupied.build.buildId).execute();

    await expect(claimGoldenSnapshotBuildBatch(
      db, '2026-07-03T00:02:00.000Z', '2026-07-03T00:07:00.000Z', 5, 10, 2,
    )).resolves.toEqual([]);
  });

  it('reserves validation capacity atomically and waits for prior clone cleanup', async () => {
    const build = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000045',
      buildId: '20000000-0000-4000-8000-000000000045', now: '2026-07-03T00:00:00.000Z',
    });
    await db.executor.updateTable('golden_snapshot_builds').set({
      status: 'running', phase: 'validation_create', attempts: 1,
      lease_expires_at: '2026-07-03T00:10:00.000Z', provider_builder_id: 945,
    }).where('build_id', '=', build.build.buildId).execute();
    await db.executor.insertInto('golden_snapshot_cleanup').values({
      cleanup_id: '50000000-0000-4000-8000-000000000045', snapshot_id: build.snapshot.snapshotId,
      build_id: build.build.buildId, resource_type: 'validation_server', provider_resource_id: 946,
      provenance_key: 'build:45:validation:1', reason: 'validation_clone_completed', status: 'queued',
      attempts: 0, next_attempt_at: '2026-07-03T00:00:00.000Z', lease_expires_at: null,
      last_error_code: null, created_at: '2026-07-03T00:00:00.000Z', completed_at: null,
    }).execute();

    await expect(reserveGoldenSnapshotValidationCreate(db, {
      buildId: build.build.buildId, validationOrdinal: 1, callbackTokenHash: 'a'.repeat(64),
      callbackExpiresAt: '2026-07-03T00:20:00.000Z', now: '2026-07-03T00:01:00.000Z', maxResources: 2,
    })).resolves.toBe(false);
    await db.executor.updateTable('golden_snapshot_cleanup').set({
      status: 'completed', completed_at: '2026-07-03T00:02:00.000Z',
    }).where('cleanup_id', '=', '50000000-0000-4000-8000-000000000045').execute();
    await expect(reserveGoldenSnapshotValidationCreate(db, {
      buildId: build.build.buildId, validationOrdinal: 1, callbackTokenHash: 'a'.repeat(64),
      callbackExpiresAt: '2026-07-03T00:20:00.000Z', now: '2026-07-03T00:02:00.000Z', maxResources: 2,
    })).resolves.toBe(true);
    await expect(db.executor.selectFrom('golden_snapshot_builds')
      .select(['pending_operation', 'callback_token_hash']).where('build_id', '=', build.build.buildId)
      .executeTakeFirstOrThrow()).resolves.toEqual({
      pending_operation: `validation:${build.build.buildId}:1`, callback_token_hash: 'a'.repeat(64),
    });
  });
  ...
  it('reclaims an expired running build without requiring a second capacity slot', async () => {
    const enqueued = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000054',
      buildId: '20000000-0000-4000-8000-000000000054',
      now: '2026-07-03T00:00:00.000Z',
    });
    await db.executor.updateTable('golden_snapshot_builds').set({
      status: 'running', phase: 'builder_create', attempts: 1,
      provider_builder_id: 854, lease_expires_at: '2026-07-03T00:01:00.000Z',
    }).where('build_id', '=', enqueued.build.buildId).execute();

    await expect(claimGoldenSnapshotBuild(
      db, enqueued.build.buildId, '2026-07-03T00:02:00.000Z',
      '2026-07-03T00:07:00.000Z', 5, 1,
    )).resolves.toMatchObject({ status: 'running', attempts: 2, providerBuilderId: 854 });
  });

  it('batch-reclaims expired running builds even when they occupy all capacity', async () => {
    const enqueued = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000055',
      buildId: '20000000-0000-4000-8000-000000000055',
      now: '2026-07-03T00:00:00.000Z',
    });
    await db.executor.updateTable('golden_snapshot_builds').set({
      status: 'running', phase: 'builder_create', attempts: 1,
      provider_builder_id: 855, lease_expires_at: '2026-07-03T00:01:00.000Z',
    }).where('build_id', '=', enqueued.build.buildId).execute();

    await expect(claimGoldenSnapshotBuildBatch(
      db, '2026-07-03T00:02:00.000Z', '2026-07-03T00:07:00.000Z', 5, 10, 1,
    )).resolves.toEqual([
      expect.objectContaining({ buildId: enqueued.build.buildId, attempts: 2, providerBuilderId: 855 }),
    ]);
  });
  ...
  it('terminalizes an active build on current-image metadata conflict without deleting that image', async () => {
    const enqueued = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
    ...
    await db.executor.updateTable('golden_snapshots').set({
      state: 'validating', provider_image_id: 952, provider_image_status: 'available',
      image_architecture: 'x86', image_disk_gb: 40,
    }).where('snapshot_id', '=', enqueued.snapshot.snapshotId).execute();
    await db.executor.updateTable('golden_snapshot_builds').set({
      status: 'running', phase: 'validation_boot', provider_builder_id: 852,
      provider_validation_id: 853, lease_expires_at: '2026-07-03T00:10:00.000Z',
    }).where('build_id', '=', enqueued.build.buildId).execute();
    ...
    await expect(db.executor.selectFrom('golden_snapshot_builds')
      .select(['status', 'phase', 'last_error_code', 'lease_expires_at'])
      .where('build_id', '=', enqueued.build.buildId).executeTakeFirstOrThrow())
      .resolves.toEqual({
        status: 'failed', phase: 'failed', last_error_code: 'provider_image_metadata_conflict',
        lease_expires_at: null,
      });
    await expect(listPendingGoldenSnapshotCleanup(db, '2026-07-03T00:03:00.000Z', 10))
      .resolves.toEqual(expect.arrayContaining([
        expect.objectContaining({ resourceType: 'builder_server', providerResourceId: 852 }),
        expect.objectContaining({ resourceType: 'validation_server', providerResourceId: 853 }),
      ]));
    await expect(listPendingGoldenSnapshotCleanup(db, '2026-07-03T00:03:00.000Z', 10))
  ...
  it('retires a freshness-expired channel snapshot only after its replacement is ready', async () => {
    ...
    const replacement = await enqueueGoldenSnapshotBuild(db, {
      ...
      buildId: '20000000-0000-4000-8000-000000000014', now: '2026-07-03T00:01:00.000Z',
    });
    await db.executor.updateTable('golden_snapshots').set({
  ...
  it('preserves the first provider action recorded for a create intent', async () => {
    const enqueued = await enqueueGoldenSnapshotBuild(db, {
      bundleVersion: 'v1', compatibility,
      snapshotId: '10000000-0000-4000-8000-000000000056',
      buildId: '20000000-0000-4000-8000-000000000056', now: '2026-07-03T00:00:00.000Z',
    });
    await db.executor.updateTable('golden_snapshots').set({
      state: 'ready', provider_image_id: 956, provider_image_status: 'available',
      image_architecture: 'x86', image_disk_gb: 40, ready_at: '2026-07-03T00:01:00.000Z',
    }).where('snapshot_id', '=', enqueued.snapshot.snapshotId).execute();
    await db.executor.insertInto('golden_snapshot_leases').values({
      lease_id: '40000000-0000-4000-8000-000000000056', snapshot_id: enqueued.snapshot.snapshotId,
      machine_id: '30000000-0000-4000-8000-000000000056', purpose: 'provision',
      target_bundle_version: 'v1', created_at: '2026-07-03T00:01:00.000Z',
      expires_at: '2026-07-03T00:11:00.000Z', released_at: null,
    }).execute();
    await createGoldenSnapshotCreateIntent(db, {
      intentId: '50000000-0000-4000-8000-000000000056',
      snapshotId: enqueued.snapshot.snapshotId,
      leaseId: '40000000-0000-4000-8000-000000000056',
      machineId: '30000000-0000-4000-8000-000000000056', purpose: 'provision',
      rolloutGeneration: 1, now: '2026-07-03T00:02:00.000Z',
    });

    await expect(markGoldenSnapshotCreateIntentAccepted(
      db, '40000000-0000-4000-8000-000000000056', 7001,
      '2026-07-03T00:03:00.000Z',
    )).resolves.toMatchObject({ providerCreateActionId: 7001 });
    await expect(markGoldenSnapshotCreateIntentAccepted(
      db, '40000000-0000-4000-8000-000000000056', 7001,
      '2026-07-03T00:03:01.000Z',
    )).resolves.toMatchObject({ providerCreateActionId: 7001 });
    await expect(markGoldenSnapshotCreateIntentAccepted(
      db, '40000000-0000-4000-8000-000000000056', 7002,
      '2026-07-03T00:03:02.000Z',
  ...
  it('serializes channel promotion and retirement through the immutable release row', async () => {
    ...
    const dbSource = await readFile('packages/platform/src/repositories/releases.ts', 'utf8');
