...
describe("device routes", () => {
  ...
  describe("POST /auth/device/approve", () => {
    ...
    it("returns a native callback handoff page after approval when requested", async () => {
      ...
      expect(successHtml).toContain('<svg class="rabbit-mark success-rabbit"');
      expect(successHtml).toContain(`.success-rabbit{color:${desktopPalette.forest}`);
      expect(successHtml).not.toContain('<div class="mark" aria-hidden="true">M</div>');
  ...
  describe("GET /auth/device", () => {
    it("renders the approval HTML and sets a CSRF cookie", async () => {
      ...
      expect(html).not.toContain('id="instance-line"');
