...
`,
  ...
  it('exposes selectable coding agent CLIs on customer hosts', () => {
    ...
    expect(toolPackInstaller).toContain('install_developer_tool()');
    ...
    expect(toolPackInstaller).toContain('codex|claude-code|opencode|pi|coding-agents|code-server|hermes|linux-tools|all');
    expect(toolPackInstaller).toContain('install_developer_tool codex');
    expect(toolPackInstaller).toContain('install_developer_tool claude-code');
    expect(toolPackInstaller).toContain('install_developer_tool opencode');
    expect(toolPackInstaller).toContain('install_developer_tool pi');
    ...
    expect(cloudInit.indexOf('add-apt-repository -y universe')).toBeLessThan(
      cloudInit.indexOf('DEBIAN_FRONTEND=noninteractive apt-get install -y build-essential ca-certificates cmatrix'),
    );
