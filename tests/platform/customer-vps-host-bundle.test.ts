...
describe('customer VPS host bundle', () => {
  ...
  it('packages an asynchronous developer tools first-boot service', () => {
    ...
    const readinessWaiter = readFileSync(join(root, 'distro/customer-vps/host-bin/matrix-wait-runtime-ready'), 'utf8');
    ...
    expect(unit).toContain('ExecStartPre=/opt/matrix/bin/matrix-wait-runtime-ready');
    ...
    expect(readinessWaiter).toContain('MATRIX_OPTIONAL_TOOLS_START_DELAY_SECONDS');
    expect(readinessWaiter).toContain('/opt/matrix/register-complete');
    expect(readinessWaiter).toContain('exit 75');
