...
Desktop badge counts may include `RuntimeSummary.attentionThreads.items.length`, but the gateway summary remains the source of truth. Do not mirror coding-agent attention state into a separate desktop-owned registry.
