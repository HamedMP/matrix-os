...
Default app manifests must only reference shipped icons in `home/system/icons/`. The preferred raster style is light iOS/macOS skeuomorphic source artwork: a full-square image with no baked-in rounded canvas, rounded-square tile, or dark/transparent corner mask, because the Matrix shell applies the final corner radius. Games must use distinct concrete icons (`2048`, `backgammon`, `chess`, `minesweeper`, `snake`, `solitaire`, `tetris`) instead of sharing `game-center`; `pomodoro` uses `pomodoro-timer`, `symphony` uses `code`, and `whiteboard` uses `whiteboard`. The default-app manifest/icon tests enforce this so new VPS homes and restored user homes do not depend on runtime icon generation for first paint.
