...
import { TerminalServerFrameSchema, type TerminalRef } from "@matrix-os/contracts";
...
export class ShellSocket {
  ...
  private buildUrl(isReconnect: boolean): string {
    ...
    return `${base}/ws/terminal/tab?workspaceId=${encodeURIComponent(this.terminalRef.workspaceId)}&tabId=${encodeURIComponent(this.terminalRef.tabId)}&client=electron&fromSeq=${fromSeq}${runtimeSuffix}${sizingSuffix}`;
  }
