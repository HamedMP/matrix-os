...
export class ShellSocket {
  ...
  private handleMessage(data: unknown): void {
    ...
    if (typeof frame.revision === "number" && frame.type !== "output" && frame.type !== "exit") {
  ...
  private handleOutput(frame: Record<string, unknown>): void {
    ...
    if (this.receivedOutput && seq <= this.lastSeqValue) return;
  ...
  private handleSnapshot(frame: Record<string, unknown>): void {
    ...
    const presentationRevision = typeof frame.presentationRevision === "number"
      ...
      : 0;
    const previousSeq = this.lastSeqValue;
    const replacesPresentation = presentationRevision > this.lastPresentationRevisionValue;
    const replacesLiveTail = this.receivedOutput && previousSeq === LIVE_TAIL_FROM_SEQ && seq < previousSeq;
    if (this.receivedOutput && seq <= previousSeq && !replacesPresentation && !replacesLiveTail) return;
    // The snapshot ANSI is a complete authoritative presentation, while the
    // existing sequence remains the server's resume watermark. Preserve that
    // watermark so this attachment's next live frame arrives at N+1 without
    // requesting replay bytes that the runtime no longer retains.
    this.lastSeqValue = replacesPresentation && previousSeq !== LIVE_TAIL_FROM_SEQ
      ? Math.max(previousSeq, seq)
      : seq;
