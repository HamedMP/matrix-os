...
export class ShellSocket {
  ...
  private openSocket(isReconnect: boolean): void {
    ...
    // Revisions are monotonic only within one attachment. A resize can move
    // this connection onto the workspace revision domain, while a reconnect
    // starts again from the tab/snapshot revision returned by `attached`.
    this.lastRevisionValue = 0;
