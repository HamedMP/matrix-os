...
export function buildGatewayUrl(baseUrl: string, path: string, runtimeSlot: string): string {
  ...
  // Personal connections belong to the signed-in actor. The shared Preview
  // machine's bearer must never select a user's Integrations or MCP account.
  if (/^pr-[1-9][0-9]{0,8}$/.test(runtimeSlot)
    && /^\/api\/(?:integrations|mcp-servers)(?:[/?]|$)/.test(path)) {
    return `${base}${path}`;
  }
