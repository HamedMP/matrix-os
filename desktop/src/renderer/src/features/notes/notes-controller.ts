...
interface FlushableNotesController {
  flush(): Promise<boolean>;
}

let activeNotesController: FlushableNotesController | null = null;

/** Registers the one canonical Notes surface that can own unsaved edits. */
export function registerActiveNotesController(controller: FlushableNotesController): () => void {
  activeNotesController = controller;
  return () => {
    if (activeNotesController === controller) activeNotesController = null;
  };
}

/** Saves dirty Notes while the current runtime credential is still valid. */
export async function flushActiveNotesBeforeIdentityChange(): Promise<void> {
  if (!activeNotesController) return;
  if (!await activeNotesController.flush()) throw new Error("notes_save_failed");
}
